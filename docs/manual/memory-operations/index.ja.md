---
title: "マニュアル/memory-operations 索引（自動生成）"
date: "2026-08-19"
type: "reference"
lang: "ja"
graph: false
excerpt: "マニュアル/memory-operations 配下の frontmatter から自動生成したフォルダ索引（OKF 段階開示）。サブフォルダ索引と直下文書への入口。"
---

# マニュアル/memory-operations 索引（自動生成）

> このファイルは `scripts/gen-spec-index.mjs` が frontmatter から生成する。手で編集しない。
> 関係は各ファイルの frontmatter `related`（型付き）が単一ソース。型: references / depends-on / implements / part-of / supersedes / refines。

総数: 1 件

## 文書

### [記憶管理マニュアル（Claude Code / Codex と anytime 拡張）](memory-operations.ja.md) `operations`

Claude Code と Codex をコーディングエージェントとして使い、Anytime Agent / Markdown / Trail の 3 拡張を導入したワークスペースで、知識と作業履歴をどこへ書き、どう読ませ、どう棚卸しするかの手順書。記憶を 4 層に分けて層ごとの寿命とロード条件を示し、層を選ぶ判断、…

関連: part-of → ../dev-operations/dev-operations.ja.md; /tech/claude-code/claude-code-memory-architecture.ja.md

