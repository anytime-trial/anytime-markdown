...
| 2 | upload markdown file | `.md` ファイルをアップロードするとエディタに反映されること | `"# Uploaded Heading\n\nUploaded paragraph content"` の .md ファイル | エディタに `"Uploaded Heading"`, `"Uploaded paragraph content"` を含む |
...
| 2 | edit markdown in source mode reflects in edit mode | ソースモードでの編集が編集モードに反映されること | ソースモードで `"# Heading from Source\n\nParagraph text"` を入力 | 編集モード: `<h1>` に `"Heading from Source"`, `"Paragraph text"` を含む |
...
| # | テスト名 | 検証内容 | IN | OUT（期待値） |
| --- | --- | --- | --- | --- |
| 1 | insert heading via slash command | `/h1` でスラッシュコマンドから Heading 1 を挿入できること | `/h1` を入力し Heading 1 を選択 | `<h1>` 要素が visible |
...
| # | テスト名 | 検証内容 | IN | OUT（期待値） |
| --- | --- | --- | --- | --- |
| 1 | outline panel shows headings | アウトラインパネルに見出しが表示されること | 見出し3つのMarkdownを入力し Outline ボタンをクリック | `"First Heading"`, `"Second Heading"`, `"Third Heading"` ボタンが visible |
