...
更新日: 2026-03-05
テスト総数: 523 テスト中 45 テスト（下記1ファイル分）
...
## [ファイル: roundTrip.test.ts（45件）](../packages/editor-core/src/__tests__/roundTrip.test.ts)
...
| # | テスト名 | 検証内容 | IN | OUT（期待値） |
|---|---------|---------|-----|--------------|
| 1 | 基本的なテーブル | パイプ記法・ヘッダー・データ行の保持 | `\| 名前 \| 年齢 \|\n\| --- \| --- \|\n\| 太郎 \| 25 \|` | `名前` `年齢` `太郎` `25` `\|` 含む |
...
| 4 | 画像 + テーブル | 画像 URL とテーブル記法の共存 | `![photo](...)\n\n\| A \| B \|\n\| --- \| --- \|\n\| 1 \| 2 \|` | `![photo](...)` `\|` `1` `2` 含む |
...
| 3 | 画像の後にテーブル | 画像 + テーブル記法の保持 | `![alt](image.png)\n\n\| A \| B \|\n\| --- \| --- \|\n\| 1 \| 2 \|` | `![alt](image.png)` `\| A \| B \|` `\| 1 \| 2 \|` 含む |
