...
| Markdown ファイルの D&D インポート | Web | 通常 | x | x | o | x | .md ファイルをエディタにドラッグ&ドロップでコンテンツ取り込み |
| 自動保存（localStorage） | 全 | 両方 | x | x | o | o | 編集内容を `STORAGE_KEY_CONTENT` に自動永続化 |
...
| 検索・置換 | 全 | 両方 | o | o | o | o | Ctrl+F で検索、Ctrl+H で置換。大文字小文字区別・単語単位・正規表現対応 |
...
| 太字・斜体・下線・取消線等 | 全 | 両方 | x | x | o | x | 下線は Ctrl+U |
| ハイライト（蛍光マーカー） | 全 | 両方 | x | x | o | x | Ctrl+Shift+H |
| インラインコード | 全 | 両方 | x | x | o | x | Ctrl+E |
| リンク挿入 | 全 | 両方 | x | x | o | x | |
...
| 日付挿入 | 全 | 両方 | x | x | o | x | `/date` で YYYY-MM-DD 形式の今日の日付を挿入 |
| YAML フロントマター | 全 | 両方 | o | o | o | o | 折りたたみ表示。Source モードで直接編集。印刷時非表示。削除は確認ダイアログ付き |
| 目次（TOC） | 全 | 両方 | x | x | o | x | 見出しから自動生成 |
| Details / Summary | 全 | 両方 | x | x | o | x | 折りたたみ可能ブロック |
| コメント挿入 | 全 | 両方 | x | x | o | x | `/comment` でカーソル位置にコメントマーカーを挿入 |
...
| 画像 D&D 挿入 | Web | 両方 | x | x | o | x | 画像 | 画像ファイルをエディタにドラッグ&ドロップで挿入 |
| 画像クリップボード貼り付け | 全 | 両方 | x | x | o | x | 画像 | Ctrl+V でクリップボードから画像を貼り付け |
| ブロック削除 | 全 | 両方 | x | x | o | x | 全ブロック | |
| ドラッグハンドル | 全 | 両方 | x | x | o | x | 全ブロック | ドラッグで位置を移動 |
| プレビュー / 構文ハイライト | 全 | 両方 | o | o | o | x | 全ブロック | コード: lowlight 構文ハイライト / Mermaid: SVG / PlantUML: 画像 / 数式: KaTeX / HTML: DOMPurify サニタイズ |
| リサイズ | 全 | 両方 | x | x | o | x | 画像 / ダイアグラム / 数式 | 右下グリップのドラッグで幅を変更。画像はキーボード操作にも対応 |
| ダブルクリック編集 | 全 | 両方 | o | o | o | x | ダイアグラム / 数式 / HTML | ダブルクリックでプレビューからブロック要素編集画面を開く |
...
| 機能 | 対応 | 比較 | Readonly | Review | WYSIWYG | Source | 対象 | 備考 |
| --- | :---: | :---: | :---: | :---: | :---: | :---: | --- | --- |
| 編集画面表示 | 全 | 両方 | o | o | o | x | 全ブロック | ツールバーの編集ボタンまたはダブルクリック（ダイアグラム / 数式 / HTML） |
| コード / プレビュー分割 | PC | 両方 | o | o | o | x | 全ブロック | ドラッグでパネル幅を変更（初期 500px、HTML は 50%）。SP は縦並び |
| ライブプレビュー | 全 | 両方 | o | o | o | x | 全ブロック | コード: シンタックスハイライト / Mermaid: SVG / PlantUML: 画像 / 数式: KaTeX / HTML: DOMPurify ライブレンダリング |
| ズーム・パン | 全 | 両方 | o | o | o | x | 全ブロック | ボタン / ホイールでズーム、ドラッグでパン |
...
| 機能 | 対応 | 比較 | Readonly | Review | WYSIWYG | Source | 備考 |
| --- | :---: | :---: | :---: | :---: | :---: | :---: | --- |
| Alt / Title 編集 | 全 | 両方 | x | x | o | x | 編集画面のツールバーから編集 |
...
| 見出し番号の自動採番 | 全 | 両方 | o | o | o | x | 設定パネルの「見出し番号」スイッチで切替（例: 1, 1.1, 1.1.1） |
...
| 機能 | 対応 | 比較 | Readonly | Review | WYSIWYG | Source | 備考 |
| --- | :---: | :---: | :---: | :---: | :---: | :---: | --- |
| コメント追加（テキスト選択） | 全 | 両方 | x | o | o | x | Ctrl+Shift+M またはバブルメニュー。選択テキストは黄色背景でハイライト |
| コメント追加（マーカー） | 全 | 両方 | x | o | o | x | テキスト未選択時にカーソル位置へマーカー挿入。`/comment` でも可 |
| コメントパネル表示 | 全 | 両方 | o | o | o | x | ツールバーのコメントアイコンで開閉 |
| コメント解決 | 全 | 両方 | x | o | o | x | コメントを解決済みにマーク |
| フィルター | 全 | 両方 | o | o | o | x | すべて・未解決・解決済みで絞り込み |
...
| --- | :---: | :---: | :---: | :---: | :---: | :---: | --- |
| 行間 | 全 | 両方 | o | o | o | o | |
| フォントサイズ | 全 | 両方 | o | o | o | o | |
| テーブル幅 | 全 | 両方 | o | o | o | x | |
| エディタ最小幅 | 全 | 両方 | o | o | o | o | |
| エディタ背景色 | 全 | 両方 | o | o | o | o | |
| スペルチェック | 全 | 両方 | o | o | o | o | ブラウザ標準スペルチェックの有効/無効（デフォルト: OFF） |
| エンコーディング変更 | 全 | 両方 | o | o | o | o | ステータスバーから UTF-8 / Shift_JIS / EUC-JP を切替。変換前に確認ダイアログ |
| 改行コード変更 | 全 | 両方 | o | o | o | o | ステータスバーから LF / CRLF を切替 |
