...
EXT_DIR="$REPO_ROOT/packages/vscode-tickets-extension"

mkdir -p "$DIST_DIR"

echo "=== Anytime Tickets: Build & Install ==="
...
echo "Packaging vsix..."
npx vsce package --no-dependencies -o "$DIST_DIR/anytime-tickets.vsix"

echo "Installing..."
bash "$REPO_ROOT/scripts/vscode-extension/_install-vsix.sh" "$DIST_DIR/anytime-tickets.vsix"
