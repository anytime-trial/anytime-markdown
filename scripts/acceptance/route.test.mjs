...
test("decideRoute: auto 無効時は auto の Level Gate を評価せず machine へ縮退する（評価順序の回帰）", () => {
  const missRates = [{ route: "auto", acceptedCount: 10, missedCount: 5, missRate: 0.5, windowDays: 14 }];
  const r = decideRoute({ score: 0.05, categories: {}, missRates, machineDecidedCount: 100, config: TEST_CONFIG });
  assert.equal(r.route, "machine");
  assert.ok(!r.reasons.some((x) => /level gate/.test(x)));
  // auto 有効 + ガード成立時は確定経路 auto に対して Level Gate が効く
  const cfg = { ...TEST_CONFIG, autoRouteEnabled: true };
  const demoted = decideRoute({ score: 0.05, categories: {}, missRates, machineDecidedCount: 30, config: cfg });
  assert.equal(demoted.route, "human");
  assert.ok(demoted.reasons.some((x) => /level gate: auto/.test(x)));
});

test("runRouting: スコア入力 3 系統が全滅なら miss-rate だけ取れても human 縮退（allMissing の回帰）", async () => {
  const repo = makeRepoWithChange("docs/notes.md");
  const reportPath = path.join(mkTmpDir("route-report-"), "r.json");
  const payloads = {
    "miss-rate": { missRates: [] },
