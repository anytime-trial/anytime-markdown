...
test("buildSandbox: fixture チケットと bare remote が作られ、push が外部へ出ない", () => {
  ...
  try {
    ...
    // workspace 識別子: ディレクトリ名 fallback と VS Code 設定（最優先）の両経路が anytime-markdown に解決する
    assert.equal(path.basename(sandbox.workspace), "anytime-markdown");
    const settings = JSON.parse(fs.readFileSync(path.join(sandbox.workspace, ".vscode/settings.json"), "utf8"));
    assert.equal(settings["anytimeAgent.tickets.workspace"], "anytime-markdown");
    assert.equal(settings["anytimeAgent.tickets.directory"], sandbox.tickets);
  } finally {
...
test("runCanary: 全 tick 正常終了でもチケットが前進しなければ canary:no-progress で fail する", async () => {
  const repo = makeRepoWithLoopChange();
  const shim = writeShim(`echo "[INFO] quiet tick"\nexit 0`);
  const reportPath = path.join(mkTmpDir("canary-report-"), "canary-report.json");
  const r = await runCanary({ commit: "HEAD", root: repo, config: SMALL_CONFIG, claudeBin: shim, reportPath });
  assert.equal(r.verdict, "fail");
  assert.ok(r.failedChecks.includes("canary:no-progress"));
});
