...
test('parseNameStatus: A/M/R を解析し D を除外・R は旧パスを保持する', () => {
  const raw = ['A\0.tickets/T-1-a.md', 'M\0.tickets/T-2-b.md', 'R100\0.tickets/T-3-old.md\0.tickets/T-3-new.md', 'D\0.tickets/T-4-d.md', ''].join('\0');
  assert.deepEqual(parseNameStatus(raw), [
    { status: 'A', oldPath: null, path: '.tickets/T-1-a.md' },
    { status: 'M', oldPath: '.tickets/T-2-b.md', path: '.tickets/T-2-b.md' },
    { status: 'R', oldPath: '.tickets/T-3-old.md', path: '.tickets/T-3-new.md' },
  ]);
});
...
test('統合: assignee: user のままのリネームでは発火しない（returns 済みチケットの誤通知回帰）', async () => {
  const ctx = await setupRepo();
  try {
    ctx.writeConfig();
    await ctx.returnTicket();
    ctx.requests.length = 0;
    await ctx.run(['mv', '.tickets/T-9-sample.md', '.tickets/T-9-renamed.md']);
    await ctx.run(['commit', '-q', '-m', 'chore(tickets): T-9 リネーム']);
    await ctx.runScript();
    assert.equal(ctx.requests.length, 0);
  } finally {
    ctx.cleanup();
