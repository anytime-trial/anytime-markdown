...
// 別リポ生成型スキル(externalRepoRefs: true)の除外。
// リグレッション: anytime-build-webapp は生成先リポジトリの `npm run dev` / `npm run gen:api` を
// 本文に含むため、本リポの package.json と照合する既定動作では恒久 fail していた(811fb7ce5)。
const EXTERNAL_REPO_SKILL_MD = [
  '---',
  'name: gen-skill',
  'description: 別リポジトリを生成するスキル',
  ...
  '更新日: 2026-07-16',
  '',
  '生成先で `npm run no-such-script` を実行し、`packages/no-such-pkg/file.ts` を作る。',
  'typo 参照 /anytime-markdown-docs/spec/x.md は引き続き検出されるべき。',
].join('\n');

test('lintSkillsDir: externalRepoRefs: true は repoPath/npm script を別リポ文脈としてスキップし typo は検出する', () => {
  const dir = mkdtempSync(join(tmpdir(), 'skill-refs-ext-'));
  try {
    mkdirSync(join(dir, 'gen-skill'), { recursive: true });
    writeFileSync(join(dir, 'gen-skill', 'SKILL.md'), EXTERNAL_REPO_SKILL_MD, 'utf-8');
    // tmp dir はリポ外(=既定で全スキップ)のため repoLocal seam で repo 文脈を再現し、flag の効果だけを見る。
    const [res] = lintSkillsDir(dir, new Set(), { docsRootAvailable: false, repoLocal: true });
    assert.deepEqual(res.missingScripts, []);
    assert.deepEqual(res.missingRefs, ['(typo? /Shared 欠落) /anytime-markdown-docs/spec/x.md']);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test('lintSkillsDir: frontmatter に externalRepoRefs が無ければ従来どおり fail する（除外の閉じ込め）', () => {
  const dir = mkdtempSync(join(tmpdir(), 'skill-refs-ext-'));
  try {
    mkdirSync(join(dir, 'gen-skill'), { recursive: true });
    writeFileSync(
      join(dir, 'gen-skill', 'SKILL.md'),
      EXTERNAL_REPO_SKILL_MD.replace('externalRepoRefs: true\n', ''),
      'utf-8',
    );
    const [res] = lintSkillsDir(dir, new Set(), { docsRootAvailable: false, repoLocal: true });
    assert.deepEqual(res.missingScripts, ['no-such-script']);
    assert.ok(res.missingRefs.includes('packages/no-such-pkg/file.ts'));
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test('lintSkillsDir: 本文中(frontmatter 外)の externalRepoRefs: true は効かない', () => {
  const dir = mkdtempSync(join(tmpdir(), 'skill-refs-ext-'));
  try {
    mkdirSync(join(dir, 'gen-skill'), { recursive: true });
    writeFileSync(
      join(dir, 'gen-skill', 'SKILL.md'),
      ['# gen-skill', '更新日: 2026-07-16', '', 'externalRepoRefs: true', '', 'npm run no-such-script'].join('\n'),
      'utf-8',
    );
    const [res] = lintSkillsDir(dir, new Set(), { docsRootAvailable: false, repoLocal: true });
    assert.deepEqual(res.missingScripts, ['no-such-script']);
...
test('anytime-build-webapp(canonical 実在時): externalRepoRefs で npm script 照合をスキップする（811fb7ce5 の回帰）', (t) => {
  const canonicalDir = join(repoRoot, '.claude', 'skills');
  const pkg = JSON.parse(readFileSync(join(repoRoot, 'package.json'), 'utf-8'));
  const rootScripts = new Set(Object.keys(pkg.scripts ?? {}));
  const results = lintSkillsDir(canonicalDir, rootScripts, { docsRootAvailable: false });
  const target = results.find((r) => r.skill === 'anytime-build-webapp');
  if (!target) {
    t.skip('anytime-build-webapp が canonical に存在しないためスキップ');
    return;
  }
  assert.deepEqual(target.missingScripts, []);
  assert.deepEqual(target.missingRefs, []);
});
