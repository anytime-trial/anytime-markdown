...
Markdown ベースのノートアプリ Obsidian の動向を調査する。当プロジェクト（ProseMirror を直接使う Markdown エディタ。`packages/markdown-core` が `prosemirror-*` に依存し、Tiptap は使っていない）の競合・参照実装・エコシステム観点で評価する。
...
- コードグラフ生成・コミュニティ要約（`activity_current_code_graphs` / community mappings / reverse-spec）
- セッション・コミット・レビューの記録と因果追跡（`messages` / `activity_session_commits` / `caravan_reviews`）
- CaravanBook グラフ検索（`search_caravan_book`）。設計書検索（markdown-catalog: 構造 + FTS + embedding）は mcp-markdown 側のみ（trail 側の markdown-catalog は 2026-08-06 に廃止）
...
3. **`anytime-analysis` スキルを起動して提案書を別ファイルで生成する。** 形式は原則 `lightweight`（改善提案）、複数案の合意形成が必要なら `rfc`。週次は知見が複数領域に跨るため、根拠を厚くする場合は `--deep`（反証付き）を付けてよい。
   ...
   - 提案書の出力先・frontmatter・テンプレートは anytime-analysis スキルに従う（`proposal/[YYYYMMDD]-[topic].ja.md` に別ファイルで保存される）。weekly レポート本体には**生成した提案書のパスと一文サマリのみ**を記載し、本文は重複させない。
4. **生成した提案書ごとにチケットを起票する。** `mcp__claude_ai_mcp-cms-remote__create_ticket` ツールを提案書 1 件につき 1 回呼び出す（チケットリポジトリの `.tickets/` へ GitHub API 経由で直接コミットされるため、ローカルの git 操作は不要）:
   - `title`: `Trail 改善提案: <提案テーマ>`
   - `description`: 一文サマリ + 提案書パス（`proposal/[YYYYMMDD]-[topic].ja.md`）+ 起点の知見（トピック番号）。実装前に提案書本体を Read するよう明記する
   - `status`: `backlog`（既定。**人が採否を判断するまで自動実行ループの対象外**に保つ — loop は `up_next` / `in_progress` のみ拾う）
   - `assignee`: `user`（提案の採否＝What の承認は人の判断。採用時に人が `agent` へ変更して着手させる）
   - `workspace`: `anytime-markdown`
   - `priority`: 原則 `medium`。効率／品質／要件適合への寄与が特に大きいと判定した場合のみ `high`
   - `creator`: `weekly-research`
   - レスポンスのチケット ID（`T-N`）を控え、出力ルールのトピック 11 表に記載する。ツールが使えない環境（ローカル mcp-cms には create_ticket が無い）・呼び出し失敗時はリトライせず、表のチケット列に「未起票（理由）」と記載してユーザーに委ねる
5. 応用余地のある知見が無かった週は提案を生成せず、「今週は Trail への応用候補なし」と明記する（チケットも起票しない）。

### 12. 開発手法・スキルへの差分（知見の還流）

トピック 11 が Trail **機能**への出口であるのに対し、本トピックは開発**手法**（global / プロジェクトの CLAUDE.md・rules）と**スキル**への出口である。トピック 11 だけでは、Claude Code の新機能や依存の破壊的変更のような「規約・スキルの記述を古くする知見」が誰にも受け取られない（2026-08-20〜09-19 の知見 32 件中 9 件が未反映のまま残った。`<docsRoot>/proposal/20260923-insight-feedback-to-skills.ja.md`）。

**手順:**

1. 今週のレポートで影響度「高」を付けた項目を列挙する。中・低は対象外（毎週の起票数を 0〜2 件に保つため）。
2. 各項目に知見テーマ（`packages/web-app/data/insight-timeline/themes.json` の id）を 1〜2 個当て、`references/insight-owners.md` でオーナー（突合する規約・スキルのパス）を引く。「オーナーなし」のテーマはここで終える。
3. オーナーのファイルを、知見の固有名詞（機能名・フラグ名・パッケージ名・版数）で Grep し、現行の記述を読む。次のどれかに当たれば**差分あり**とする。
   - 規約・スキルが、知見で変わった前提を書いている（例: 撤去された機能を手順に使う、変わった既定値を前提にする）
   - 知見が新しい手段を出したのに、同じ目的の手書き手順が残っている（例: 能力を API で実測できるのに手書き表を使う）
   - 依存の破壊的変更に対して、オーナーのスキルに互換の指針が無い
4. 差分ありの項目ごとにチケットを起票する。提案書は書かない（改善チケットの滞留が既知の問題で、提案書の量産は滞留を増やす。`<docsRoot>/proposal/20260819-improvement-ticket-stagnation.ja.md`）。起票はトピック 11 手順 4 と同じ `create_ticket` を使い、次だけを変える。
   - `title`: `知見の還流: <オーナーのスキル名または規約名> — <知見の要点>`
   - `description`: 次の 4 行のみ。**変更対象**（オーナーのパスと行番号）/ **現行の記述**（逐語で 1〜2 行）/ **知見**（レポートの該当項目と一次ソース URL）/ **期待する修正**（1 文）
   - `priority`: `medium`。依存のセキュリティ修正と、全セッションに効く global 規約の食い違いだけ `high`
   - `creator`: `weekly-research`
5. 差分なしの項目は起票しない。レポートの表に「差分なし」と書いて残す（突合した事実を残し、翌週の再突合を省く）。

**やらないこと:** オーナーのファイルを本スキル内で直接編集しない。週次調査はチケットを起票するところまでで、修正は人が採否を決めてから行う（What の承認は人。global `CLAUDE.md`「承認の対象」）。

## 出力ルール

- 更新があった項目のみ記載する（変更なしのモジュールは「変更なし」セクションにモジュール名のみ列挙）
- 各項目にソース URL を含める
- 影響度（高/中/低）を判定し、当プロジェクト（ProseMirror を直接使う Markdown エディタ。`packages/markdown-core` が `prosemirror-*` に依存し、Tiptap は使っていない）への影響があれば明記する
...
- トピック 11（Trail 機能改善提案）は、提案書本体を `anytime-analysis` で**別ファイル生成**し、weekly レポートには `## Trail 機能改善提案` セクションを設けて以下のみ記載する（本文は提案書側に集約し重複させない）:

  | 提案テーマ | 起点の知見（トピック番号） | 形式 | 提案書パス | チケット |
  | --- | --- | --- | --- | --- |
  | コミュニティ要約検索に reranker 導入 | 10（潮流）/ 4（RAG） | lightweight | `proposal/[YYYYMMDD]-[topic].ja.md` | T-NN |

  - チケット列には生成手順 4 で起票したチケット ID を記載する。起票できなかった場合は「未起票（理由）」を記載する
  - 応用候補が無かった週は「今週は Trail への応用候補なし」と1行記載する
- トピック 12（開発手法・スキルへの差分）は `## 開発手法・スキルへの差分` セクションに次の表で記載する。影響度「高」の項目が無かった週は「今週は影響度「高」の知見なし」と 1 行記載する:

  | 知見（影響度「高」） | テーマ | オーナー | 突合結果 | チケット |
  | --- | --- | --- | --- | --- |
  | mermaid 12.0.0 が ELK を既定化 | `dev-tooling` | `packages/vscode-markdown-extension/skills/anytime-mermaid/SKILL.md` | 差分あり（レイアウト固定の指針なし） | T-NN |
  | Next.js 16.3.3 の Critical 修正 | `dev-tooling` | `package.json` | 差分なし（16.3.5 適用済み） | — |
...
週次レポートと、トピック 11 で `anytime-analysis` が別ファイル生成した Trail 改善提案書は **論理単位ごとに分けて 2 コミット**する（提案を生成しなかった週はレポートの1コミットのみ）。
