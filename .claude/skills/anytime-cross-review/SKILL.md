...
`git diff <base>..HEAD --stat` で対象を確認する（既定 base=develop）。

### 2. Round 1 — 独立デュアルレビュー（並行）

- **Claude**: `pr-review-toolkit:code-reviewer` subagent に diff レビューを依頼する（`review-finding-format` 出力。`superpowers:requesting-code-review` と同様、subagent session 経由で memory_reviews に ingest され reviewer=`pr-review-toolkit:code-reviewer`）。
- **Codex**: `node .claude/skills/anytime-cross-review/codex-review.cjs --base <base>` を実行する。
  - stdout = レビュー本文（`### N.` 形式）、stderr に `findings=N maxSeverity=...`。
  - exit 0 = 成功 / 2 = codex 失敗 / 3 = read-only 逸脱（codex がファイルを変更）。
  - **exit≠0 は Codex 不在として degrade**: Claude 単独レビューで続行し、統合サマリに「Codex レビュー欠落（理由）」を明記する（グレースフルデグラデーション）。

### 3. Round 2 — 相互検証（adversarial）

- **Claude が Codex 指摘を検証**: Codex の各指摘を real / false-positive / 要追加情報 に分類する（diff を持つ Claude が判定）。
- **Codex が Claude 指摘を検証**: Claude 指摘を列挙したプロンプトを `codex exec --dangerously-bypass-approvals-and-sandbox -`（review でなく exec）に渡し、各指摘を confirm / refute させる。出力は `<<<CROSS-REVIEW-START>>>` / `<<<CROSS-REVIEW-END>>>` で挟むよう指示し、センチネル間を採用する。codex 失敗時は degrade。

### 4. 記録

- **Codex 指摘** → `/Shared/anytime-markdown-docs/review/<YYYYMMDD>-<branch>-codex.ja.md` を作成:
  - frontmatter: `title` / `date` / `type: review` / `lang: ja` / `reviewer: codex` / `severity: <maxSeverity>`（ラッパ stderr の値）。
  - 本文: codex-review.cjs の stdout（`review-finding-format`）。`anytime-markdown-output` 準拠。
...
- `/Shared/anytime-markdown-docs/review/<YYYYMMDD>-<branch>-cross-review.ja.md`（`type: report`）に出力: サマリ / 合意指摘（採用） / 係争（人手判断） / 採用アクション。**finding は再 ingest しない**（重複防止のため review でなく report）。
...
- review-finding-format 逸脱で Codex 指摘が取りこぼされうる（件数突合で検知）。
