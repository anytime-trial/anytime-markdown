...
- **Claude**: `pr-review-toolkit:code-reviewer` subagent に diff レビューを依頼する（`anytime-review` 出力。`superpowers:requesting-code-review` と同様、subagent session 経由で memory_reviews に ingest され reviewer=`pr-review-toolkit:code-reviewer`）。
...
- **Codex 指摘** → `/Shared/anytime-markdown-docs/review/<YYYYMMDD>-<branch-slug>-codex.ja.md` を作成:
  ...
  - 本文: codex-review.cjs の stdout（`anytime-review`）。`anytime-markdown-output` 準拠。
...
- anytime-review 逸脱で Codex 指摘が取りこぼされうる（件数突合で検知）。
