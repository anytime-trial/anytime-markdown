...
> [!IMPORTANT]
> ローカル SQLite（拡張機能の `activity.db`）には本規約を適用しない。activity.db は既存データ保持が前提のため `ALTER TABLE` / 12-step migration を使う（`sqlite-table-definition-trail-activity` および global `sqlite-table-definition` を参照）。
...
- `sqlite-table-definition-trail-activity` — activity.db（ローカル SQLite）側の規約。本スキルとは方針が異なる
