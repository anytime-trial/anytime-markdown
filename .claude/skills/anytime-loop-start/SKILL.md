...
7. **委譲起動（別セッション実行）**: チケット本文の作業を子セッションへ委譲する。
   ...
   3. 起動はコード（ランチャースクリプト）とデータ（委譲プロンプト）を**別ファイルに分離**して行う。委譲プロンプトはバッククォート・クォート・`$` を含む長い Markdown であり、シェルコマンド文字列へ直接埋め込むと本文内容に依存して引用が崩れる（`bash -c '...'` 内のバッククォートが command substitution 化した実害あり）。プロンプト本文をシェルにも argv にも一度も通さないことを不変条件とする:
      ...
      **`--add-dir <docsRoot>` も必須**（`<docsRoot>` はプロジェクト CLAUDE.md の「ドキュメント保存先（docsRoot）」節の値。tick 起動前の §0 で解決してテンプレートへ展開する）。§1.1 のとおり成果物ドキュメントは docsRoot 配下へ保存する契約であり、付けないと子は成果物を規約どおりの場所へ書けず、チケット本文へ貼る逸脱かエラーで止まる。docsRoot の git はローカルコミットまで（push しない）。
   ...
   5. 委譲プロンプトに含める 6 点（グローバル CLAUDE.md の委譲ルール準拠）:
      ...
      - **完了条件**: 全サブタスク完了（または本文の完了条件充足）→ 成果物ドキュメントは docsRoot へ保存しチケットへリンクを記載（§1.1）→ `status: in_review` 遷移と §1.2 の手離し手順（`actual` 加算・`assignee: user` 返却）をコミット・push
...
- **成果物ドキュメント（検討結果・設計書・提案・レポート等）はチケット本文へ貼らず、ワークスペースの規約で規定された保存先（プロジェクト CLAUDE.md の `docsRoot` 配下・type 別フォルダ）へ保存し、チケットにはそのリンク（`<docsRoot>` からのパス）を記載する**。チケットは状態と参照の正本であり、ドキュメントの正本は docsRoot 側に置く（本文へ貼るとチケットが肥大し、docsRoot の索引・related グラフからも外れる）。docsRoot リポジトリはローカルコミットまでとし push しない（§3 の push 原則はチケットファイルのみ）。例外: 数行で収まる要約・結論はチケットへ直接書いてよい（リンク先の要約として併記するのが望ましい）。
