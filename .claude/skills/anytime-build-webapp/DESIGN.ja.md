...
category: "skill"
excerpt: "要求から Next.js（T3 Stack）フルスタック Web アプリの MVP を生成する汎用オーケストレータスキルの設計書。ベースリポジトリ anytime-lab をクローンしてリネームし、その上に T3 を重ね合わせる。--devcontainer で Dev Container ファイル一式を同梱テンプレから生成できる。画面デザインは参考 URL または DESIGN.md ファイルから適用可能。"
clarity: 90
---
...
モードと直交するフラグとして `--devcontainer` を持つ。指定時は `.devcontainer/devcontainer.json` を
スキル同梱テンプレ（`scaffold/devcontainer-files/`）から生成し、`docker-compose.yml` / `Dockerfile` は
不在時のみ生成する。既存 `devcontainer.json` は確認のうえ上書きする（`scaffold/devcontainer.md`）。
...
│   └── overrides.md            # スタック固定の方針と将来追加手順
└── scaffold/
    ├── base-repo.md            # ベースリポ仕様（クローン元・リネーム規則）
    ├── rename-map.json         # 文字列置換マップ
    ├── devcontainer.md         # --devcontainer の生成手順・上書き確認・検証
    └── devcontainer-files/     # Dev Container テンプレ (devcontainer.json / docker-compose.yml / Dockerfile)
```
...
OpenHands `/onboard` 方式に従い、質問数 4 で打ち切り、各 ≤ 2 文、選択肢提示優先。
...
| Q4 | 画面デザインの参照源（無し / 参考 URL / DESIGN.md ファイルパス） | 無し（標準 Tailwind スタイル） |

スタック選択の質問は持たない（Next.js / T3 固定）。


主要画面の構成は Q2 の主要エンティティから自動生成する（一覧 / 詳細 / 新規作成）。

Q4 の回答により Phase 4D（Apply Design Tokens）の挙動が分岐する（第 8 章参照）。


### 5.1. 打ち切り条件


- Q1〜Q3 が明確で Q4 が CLI 引数で指定済みなら、対話質問はそこまでで打ち切る
- 1 問で 3 つ以上の情報が出てきた場合、次質問をスキップする
- 4 問終了時点で曖昧な点があれば「想定 X で進めます。違えば指示してください」と表明して継続する
...
`--design-url` または `--design-file` 指定時は Q4 をスキップし、Phase 4D で参照源として使用する。
...
flowchart TD
    ...
    P1["Phase 1: Interview<br/>(skill 本体)<br/>4 問 → 回答収集<br/>→ requirements.md 生成"] --> A1
    A1{"要件承認?<br/>(What の承認)"}
    A1 -- No --> P1
    A1 -- Yes --> P2
...
Q4 / CLI 引数の値で参照源を判定し、`design-md` スキルを介してデザイントークンを抽出・適用する。
...
スタックは分岐しない。常に `stacks/_frontend-next.md` + `stacks/t3-default.md` を適用する。

| 要求 | 挙動 |
| --- | --- |
| 指定なし | `stacks/t3-default.md` + `stacks/_frontend-next.md` を適用 |
| 別スタック（`Python BE で` 等） | 「本スキルは Next.js 専用です。T3 Stack で進めますか?」を 1 回確認し、否なら中断 |


### 8.2. 初期リリース範囲


`t3-default` のみを実装する。`hono-be` / `rails-hotwire` は使用実績が出てから追加する（YAGNI）。\
`python-be`（FastAPI）は 2026-08-22 にユーザ指示で廃止し、関連ファイルを削除した（git 履歴から復元可能）。
...
- **既存ファイル上書きガード**: `--new-dir` 指定時のみ CWD 空チェックを行う。in-place モードでは既存 `.devcontainer/devcontainer.json` を破壊しない（`inPlaceExcludes` で保護）。`--devcontainer` 指定時のみ上書きし得るが、対象一覧を提示して `AskUserQuestion` で確認する
- **Git 自動 commit 禁止**: 初期 commit のみ作成し、push は一切行わない
- **`main` 直 push 禁止**: 仮に push 指示があっても `master` / `main` 直 push はガード
- **Docker daemon 存在チェック**: `--new-dir` 指定時のみ `docker info` で接続確認。in-place モードは skip（Dev Container 内では Docker daemon 非到達のため）。`--devcontainer` はファイル出力のみのため Docker を要求しない
...
- **回帰テスト**: 4 問の質問順・打ち切り条件をスキル更新時に再実行で検証
...
| 項目 | 要件 |
| --- | --- |
| 実行時間 | 4 問インタビュー含めて 1 セッション 30 分以内（インタビュー + 要件承認 10 分 + 実装 20 分） |
...
- スタック追加（`hono-be` / `rails-hotwire`）。`python-be` は 2026-08-22 に廃止
...
- スタック上書き機構は 2026-08-22 に廃止（Next.js 専用化）。実装は `t3-default` のみ
- 4 問の文面（日本語 / 英語の切替条件）が未確定
