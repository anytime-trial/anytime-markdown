...
> [!IMPORTANT]
> 汎用 SQLite 規約は global `~/.claude/skills/sqlite-table-definition/SKILL.md` を参照。本スキルは trail-activity 固有の配置・運用差分のみを定義する。

## 適用範囲（trail-activity 固有のパス）

| 対象 | 場所 |
| --- | --- |
| 新規テーブル定義 | `packages/trail-activity/src/domain/schema/tables.ts` への CREATE TABLE 追加 |
