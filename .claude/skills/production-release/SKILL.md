...
- Step 5 の失敗調査など、判断を要する局面で一時的に effort を上げるのは可。モデル自体の切替は行わない。
...
for d in packages/*/; do
  ...
  ( cd "$d" && npm test --silent ) || echo "FAIL $d"   # ← 失敗パッケージを必ず記録する
done
```

`vscode-markdown-extension` の test だけは `vscode-test` を起動し、コンテナ環境ではネットワークエラーになる（スクリプト側で握り潰す作りになっている）。ここは exit 0 でも通過根拠にしない。

```bash
# 5-3: E2E・カバレッジ・ビルド
cd packages/web-app && E2E_COVERAGE=1 npx playwright test --project=chromium
cd packages/web-app && npx next build
# 拡張機能のビルドは 6 本すべて（リリース対象外の系統も依存の影響を検出するため回す）
root=packages
for p in vscode-markdown-extension vscode-graph-extension vscode-trail-extension \
         vscode-database-extension vscode-agent-extension vscode-history-extension; do
  ( cd "$root/$p" && npx webpack --mode production ) || echo "FAIL $p"
done
```
...
[ ] tsc --noEmit 通過
[ ] npm run lint 通過
[ ] npm run check-skills 通過
[ ] jest 設定を持つ全ワークスペースを実測列挙し、ユニットテスト全通過（固定リストで済ませない）
[ ] 変更した共有パッケージ（vscode-common 等）の波及先を grep で確認し、bump 対象に漏れがない
