...
# 注: markdown-core は vendored tiptap でテスト無し（`--passWithNoTests` を付けないと exit 1）。
#     エディタコアのテスト実体は markdown-viewer / markdown-rich にある。
cd packages/markdown-viewer && npx jest --coverage --maxWorkers=1
cd packages/markdown-rich && npx jest --coverage --passWithNoTests --maxWorkers=1
...
[ ] npm test 全ユニットテスト通過（markdown-viewer / markdown-rich）
