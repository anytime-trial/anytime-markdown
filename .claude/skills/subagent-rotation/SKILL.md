...
■ 起動
  /subagent-rotation [policy=...] <タスク概要と段階リスト>
  例) /subagent-rotation packages/foo の脱 any を5ファイル、肥大したら切り替えて
      /subagent-rotation policy=always-fresh 各章を独立に要約して
...
外部ファイル。compact 後のオーケストレータはこれを読み戻して**ループ手順3 から再開**する。これは
サブエージェントの compact-seed（`buildSeedPrompt`）と同じ「圧縮ステートで継続」を**メイン session 自身に
適用**したもので、用途(a)/(b)/(c) が同一 `HandoffState` を共有する設計の帰結。
