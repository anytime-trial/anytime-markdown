...
// 公開エントリ index と、それが re-export する MindmapViewerElement の .d.ts を検査する。
// （normalizeGraphInput.d.ts は exports マップ非公開かつ index から未参照のため対象外）
it.each(['index.d.ts', 'MindmapViewerElement.d.ts'])('公開 %s に graph-core への import が無い（自己完結）', (file) => {
