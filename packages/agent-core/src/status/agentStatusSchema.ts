...
//
// summary / summary_at は本タスクでは「列だけ予約」し、書き込み経路は作らない。後から STRICT テーブルへ
// 列を追加すると 12-step テーブル再作成が必要になるため、先に確保しておく（sqlite-table-definition.md §9）。
...
export const CREATE_AGENT_SESSIONS = `CREATE TABLE IF NOT EXISTS agent_sessions (
