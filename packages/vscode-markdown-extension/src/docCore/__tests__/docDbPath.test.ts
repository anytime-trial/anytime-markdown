...
describe('resolveDocDbPath', () => {
  it('returns the default path under the workspace root when unconfigured', () => {
    const warn = jest.fn();
    expect(resolveDocDbPath(WS, undefined, warn)).toBe(path.join(WS, '.anytime', 'markdown', 'catalog.db'));
    expect(warn).not.toHaveBeenCalled();
  });

  it('returns the default path when configured is blank', () => {
    const warn = jest.fn();
    expect(resolveDocDbPath(WS, '   ', warn)).toBe(path.join(WS, '.anytime', 'markdown', 'catalog.db'));
    expect(warn).not.toHaveBeenCalled();
  });
  ...
  it('falls back to the default path and warns when configured escapes the workspace root via traversal', () => {
    const warn = jest.fn();
    expect(resolveDocDbPath(WS, '../../etc/evil.db', warn)).toBe(path.join(WS, '.anytime', 'markdown', 'catalog.db'));
  ...
  it('falls back to the default path and warns when configured is an absolute path outside the workspace root', () => {
    ...
    expect(resolveDocDbPath(WS, outside, warn)).toBe(path.join(WS, '.anytime', 'markdown', 'catalog.db'));
    expect(warn).toHaveBeenCalledTimes(1);
