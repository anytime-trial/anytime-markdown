...
### 追加
- ツリービューのコンテキストメニューに「ファイル名をコピー」を追加
- ファイルオープン時の自動リロードをデフォルトで有効化

### 変更
- CI: publish ジョブで CI の VSIX アーティファクトを再利用（二重ビルドを排除）
- CI: publish 時に GitHub Release を自動作成（VSIX 添付）
- CI: `.node-version` ファイルで Node.js バージョンをローカル・CI で統一
- CI: `.gitattributes` で改行コードを統一（`eol=lf`）
- CI: Daily Build のスケジュールを JST 5:00 に変更

### 修正
- ツリービューのドラッグ&ドロップがコピーではなく移動になるよう修正

## [0.7.6] - 2026-03-22

### 追加
- スラッシュコマンド: mermaid/PlantUML/数式/HTML/GIF ブロック作成時に全画面編集ダイアログを自動表示
- スラッシュコマンド: フロントマターが正しい `---` フェンス形式で出力されるように修正
- スラッシュコマンド: 脚注が1始まりの連番で採番され、末尾に脚注定義を自動追加
- 引用ブロックで Tab/Shift+Tab によるネスト操作（最大6段）
- エディタ内の Tab キーによるツールバーへのフォーカス移動を抑制
- /docs/view ページにドキュメント名付きパンくずナビゲーションを追加
- next-auth v5 (Auth.js) への移行（型安全なセッション管理）

### 変更
- アドモニションのスラッシュコマンドラベルを簡潔化（日本語: 「コールアウト」削除、英語: 「Admonition」に変更）

### 修正
- アドモニションのスラッシュコマンドで admonitionType 属性が正しく設定されない問題を修正
- HTML ブロック全画面プレビューの背景色がエディタテーマと不一致だった問題を修正
- ウェブサイトのアクセシビリティ改善（コントラスト比、aria-hidden、キーボード操作対応）
- デッドコード削除（LandingPage/LandingBody）、デバッグ用 console.warn 削除、globalThis 統一
- CTA ボタンを共通コンポーネント VsCodeCtaButtons に抽出
- Hero 見出し構造の簡素化（単一 h1 + span 子要素）
...
- CI にバンドルサイズレポートを追加
- VS Code API 型スタブを追加し型安全性を向上
- localStorage ラッパーで quota 超過時にコンソール警告
...
- ブロック移動・複製ショートカットを VS Code のみ有効に変更
- ツールバー高さを 44px に固定
