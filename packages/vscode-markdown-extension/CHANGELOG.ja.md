...
- 同梱スキルを更新した。`anytime-mermaid` に図へ layout / theme / look を書かない規約を足し（テーマはアプリがダーク/ライトで切り替えるため）、サブエージェントが CLAUDE.md を継承しないとしていた記述を実測に合わせて直した。

### Diagram Core (diagram-core / diagram-viewer)

- 図の向き（左→右 / 上→下）を設定できるようにし、折れ線の取り付きと折れる位置を向きから決めるようにした。上→下では自動配置も世代＝行に組み替える。同じ列・行に並ぶ 2 つは向かい合う面の中央どうしを結ぶ。
- 子の居る夫婦の婚姻線・3 人目以降の親の線が折れ線で消える退行、縮めた図で破線が実線に見える不具合、選んだ線の中点の取っ手が ＋ の下に隠れる不具合を直した。
...
- `rootDir` の解決を明示化した。`ANYTIME_MARKDOWN_ROOT` があればそれ、無ければプロセスの cwd を使う（`.mcp.json` 経路は生成時のワークスペースが陳腐化するため環境変数を渡さない）。解決したルートが実在しない場合は stderr へ警告を出す（従来は実在しないルートのまま静かに起動していた）。
...
- ドキュメント検索データベースを `doc-core.db` から `catalog.db` へ改名し、パス解決を新設の `docDbPath` へ集約した。既存の `doc-core.db` は拡張が初めて開いた時点で自動的にリネームされる。
...
- Ctrl/Cmd + ホイールで全体ズームができるようになり、コードブロック編集ダイアログにコードペインの折りたたみ/展開アイコンを追加しました。
...
- 同梱スキルインストーラがスキル dir 全体を再帰コピーするようにし、`references/` 等のサブファイルを配布ターゲットへ配置するようにしました（従来は `SKILL.md` のみ配置し、参照ファイルがユーザー環境へ届かなかった）。
- 同梱スキル `anytime-doc-authoring` の技術記事テンプレートの見出しプレースホルダを内容特定型（「なぜそうなるか（内部の仕組み）」）へ改め、文章規範の LLM 口調排除方針と整合させました。文章規範の出典に、派生物である旨とライセンス確認の一文を追記しました。
...
- git タイムラインビューのコミット日時をローカルタイムゾーンで表示するようにしました。WSL の Extension Host は `TZ=UTC` で動くため、`Date` のローカル getter が UTC 値を返していました。
...
- GitHub で開いた文書に限りノート網パネルを webview に表示（閲覧専用）。

### 変更

- 同梱の mcp-markdown が重複見出しに対して曖昧エラーを返し、update 系ツールが実施サマリを返すよう変更。
- mcp-tactics スキルを実装と同期し、書き側の罠を追記。

### エディタコア (markdown-core)

- ノート網パネルをエディタのサイドツールバーへ配線。フロントマターを編集/レビューモードで表示。
