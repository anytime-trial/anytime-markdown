...
- `title` は「<pkg> 機能仕様書」「<pkg> E2E シナリオ」。`updated` は編集当日。`type: "spec"` には `c4Scope` を付与（2.3 参照）。
- 追加・更新・改名・削除のたびに同フォルダ索引を再生成する（2 章の索引ルール。`npm run spec:index`）。
