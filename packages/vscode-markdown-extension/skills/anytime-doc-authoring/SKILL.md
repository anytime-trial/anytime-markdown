...
| スキル | 担当 |
| --- | --- |
| **anytime-doc-authoring**（本スキル） | 何を書くか（type 別の記載内容・フォルダ構成・索引運用）＋どういう文章で書くか（文章規範。§1.4・`references/writing-standards.ja.md`） |
...
- 指摘の書式（`### N. タイトル` + 重大度/カテゴリ/対象 + 行頭 `問題:`/`提案:` マーカー）は `anytime-trail-review` スキルに従う（trail trail-caravan-book ingest が機械読取する契約）。
