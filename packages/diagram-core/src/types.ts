 ...
 * （`LR` は左から右、`TB` は上から下）。
 *
 * 向きは**人物の並びと折れ線の取り付き**を決める。自動配置は `TB` で世代を行・兄弟を列に置き
 * （`orientDiagramChart`）、画面で向きを切り替えると手で置いた升目も列と行を入れ替える
 * （`transposeDiagramPlacements`）。線だけに効かせると、上→下にしても世代が横に並んだまま線が回り込む。
