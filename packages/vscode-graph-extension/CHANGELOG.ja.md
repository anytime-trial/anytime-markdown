...
- `rootDir` の解決を明示化した。`ANYTIME_GRAPH_ROOT` があればそれ、無ければプロセスの cwd を使う。解決したルートが実在しない場合は stderr へ警告を出す。
...
- Marketplace アイコンを Anytime 拡張で共通のラクダ + ダイヤ型デザインへ差し替えた。
...
- 共起ネットワークビューアがホストからのノード選択・ハイライト（`selectNode`）に対応し、カード表示で縦に並べる枚数を選べるようにした。

### Graph Core (graph-core)

- DSL のパーサ・シリアライザ、SVG エクスポート、レイアウトプリセットの高複雑度関数を分割した。挙動は変えていない。

## [1.2.0] - 2026-08-09

### Graph Core (graph-core)

- 共起ファイルが座標の計算主体（`layout.source`）を宣言できるようになり、サーバ供給の座標はアルゴリズム版数を照合せずに利用する。
- `layout` ブロックの構造検査を追加した。

## [1.1.0] - 2026-08-08

### 追加

- 共起ネットワークビューアに card スキンを追加した。ノードをカード形式で配置でき、トグルでカード表示とグラフ表示を切り替えられる。
...
- 最初の安定版。同梱する共起ビューアとグラフエンジンの公開面が固まったため 1.0.0 とした。

### Graph Core (graph-core / cooccurrence-viewer)

- 共起ビューアを Web Component 化し、配布バンドルを追加
- 編集モードを追加し、図から語・共起を直接足せるようにした（追加ポップアップ・書き換え系コントロールの一括無効化・サイドアイコン列のトグル）
- サブクラスタ（クラスタを 1 段細分）を追加し、絞り込みの対象にした
- 絞り込みの共起 2 条件と最小頻度をスライダーへ統一。クラスタ / スライス一覧に一括選択「すべて」を追加
- OZ 3D v2: ピル選抜・曲線ストリーム・巨大円背景・クラスタ見出しを刷新し、スライスをまたぐ同一語を柱として束ねる
- クラスタパレットを 4 色相系統 × 2 段へ組み替え、ライト / ダーク共通の配色にした
- クラスタ単位のレーン表示を、スライス軸と直交する軸へ追加
- ミニマップの見た目を trail-viewer の C4 ミニマップへ揃えた
- 修正: 高 DPI で canvas が右パネルを覆う / 語一覧からの選択が OZ 3D へ伝播しない / OZ のダーク・ライトで共起の線が潰れる・溶ける / 可動域が狭いとき刻みが 0 になる

## [0.13.0] - 2026-07-30

### 追加

- `mcp-graph` MCP サーバーを拡張へ同梱し、ワークスペースの `.mcp.json` に未登録なら自動登録するようにした。手動登録コマンドと MCP サーバープロバイダを併せて追加。サーバーのルートディレクトリはプロセスの作業ディレクトリ（拡張ホストを指す）ではなく環境変数 `ANYTIME_GRAPH_ROOT` で渡す。

### 修正

- `ts-loader` の `configFile` を絶対パスで固定し、親方向探索がルートの `tsconfig` を掴むことによる間欠的なビルド失敗を解消した。
- 未ビルドの作業ツリーで回帰テストが落ちるのを直した。
