...
//
// mkdtempSync の一時ディレクトリに caravan-book.db / activity.db を作ることで、テスト中の
...
/** activity.db 側に必要なセッション由来テーブルを作って開いたまま返す（シード用ハンドル）。 */
function createSeedableTrailDb(trailDbPath: string): BetterSqlite3Database {
  const trail = openBetterSqlite3(trailDbPath);
  // 本番の activity.db と同じ FK OFF（better-sqlite3 は既定 ON。repos を張らない部分シードを許す）
...
export function createUninitializedFlightRecordDb(): { db: FlightRecordDatabase; cleanup(): void } {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'flight-record-db-'));
  return {
    db: new FlightRecordDatabase(path.join(tempDir, 'caravan-book.db'), { trailDbPath: path.join(tempDir, 'activity.db') }),
    cleanup(): void {
      fs.rmSync(tempDir, { recursive: true, force: true });
    },
  };
