...
class FakeRemoteStore implements IRemoteTrailStore {
  ...
  coverageRows: Array<{ repo_id?: number; package: string; file_path: string; lines_total: number; lines_covered: number; lines_pct: number; statements_total: number; statements_covered: number; statements_pct: number; functions_total: number; functions_covered: number; functions_pct: number; branches_total: number; branches_covered: number; branches_pct: number; updated_at: string }> = [];
  releaseCoverageRows: Array<{ release_id?: number; package: string; file_path: string; lines_total: number; lines_covered: number; lines_pct: number; statements_total: number; statements_covered: number; statements_pct: number; functions_total: number; functions_covered: number; functions_pct: number; branches_total: number; branches_covered: number; branches_pct: number }> = [];
  codeGraphRows: Array<{ repo_id: number; graph_json: string; generated_at: string; updated_at: string }> = [];
  codeGraphCommunityRows: Array<{ repo_id: number; community_id: number; label: string; name: string; summary: string; mappings_json: string | null; stable_key: string; generated_at: string; updated_at: string }> = [];
  releaseCodeGraphRows: Array<{ release_id: number; graph_json: string; generated_at: string; updated_at: string }> = [];
  releaseCodeGraphCommunityRows: Array<{ release_id: number; community_id: number; label: string; name: string; summary: string; stable_key: string; generated_at: string; updated_at: string }> = [];

  async unsafeClearCurrentCoverage(): Promise<void> { this.coverageRows = []; }
  async upsertCurrentCoverage(rows: readonly { repo_id?: number; package: string; file_path: string; lines_total: number; lines_covered: number; lines_pct: number; statements_total: number; statements_covered: number; statements_pct: number; functions_total: number; functions_covered: number; functions_pct: number; branches_total: number; branches_covered: number; branches_pct: number; updated_at: string }[]): Promise<void> {
  ...
  async upsertReleaseCoverage(rows: readonly { release_id?: number; package: string; file_path: string; lines_total: number; lines_covered: number; lines_pct: number; statements_total: number; statements_covered: number; statements_pct: number; functions_total: number; functions_covered: number; functions_pct: number; branches_total: number; branches_covered: number; branches_pct: number }[]): Promise<void> {
  ...
  async upsertCurrentCodeGraphs(rows: readonly { repo_id: number; graph_json: string; generated_at: string; updated_at: string }[]): Promise<void> {
    this.codeGraphRows.push(...(rows as typeof this.codeGraphRows));
  }
  async upsertCurrentCodeGraphCommunities(rows: readonly { repo_id: number; community_id: number; label: string; name: string; summary: string; mappings_json: string | null; stable_key: string; generated_at: string; updated_at: string }[]): Promise<void> {
  ...
  async upsertReleaseCodeGraphs(rows: readonly { release_id: number; graph_json: string; generated_at: string; updated_at: string }[]): Promise<void> {
    this.releaseCodeGraphRows.push(...(rows as typeof this.releaseCodeGraphRows));
  }
  async upsertReleaseCodeGraphCommunities(rows: readonly { release_id: number; community_id: number; label: string; name: string; summary: string; stable_key: string; generated_at: string; updated_at: string }[]): Promise<void> {
  ...
  async deleteManualElement(repoId: number, elementId: string): Promise<void> {
    this.elements = this.elements.filter(e => !(e.id === elementId && (e as ManualElement & { _repo: number })._repo === repoId));
  }
  ...
  async deleteManualRelationship(repoId: number, relId: string): Promise<void> {
    this.relationships = this.relationships.filter(r => !(r.id === relId && (r as ManualRelationship & { _repo: number })._repo === repoId));
  }
  ...
  async deleteManualGroup(repoId: number, groupId: string): Promise<void> {
    this.groups = this.groups.filter(g => !(g.id === groupId && (g as ManualGroup & { _repo: number })._repo === repoId));
  }
...
describe('SyncService.syncManualElements', () => {
  it('pushes local-only elements to remote', async () => {
    ...
    await sync.syncManualElements(repoId, 'repo-a');
    const remoteElems = await remoteStore.listManualElements(repoId);
  ...
  it('pulls remote-only elements to local', async () => {
    ...
    const remoteStore = new FakeRemoteStore();
    const repoId = (localDb as unknown as { repoIdForName(n: string): number }).repoIdForName('repo-a');
    await remoteStore.upsertManualElement(repoId, {
