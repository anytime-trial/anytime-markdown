...
function seedCommit(
  ...
  inner.run(
    `INSERT OR IGNORE INTO activity_session_commits (session_id, commit_hash, commit_message, committed_at, author) VALUES (?, ?, '', ?, ?)`,
...
describe('GET /api/bus-factor (Phase 6 S5-B)', () => {
  ...
  it('ファイル単位の属人度を返す（単独著者は score 1.0）', async () => {
    for (const [i, hash] of ['h1', 'h2', 'h3', 'h4', 'h5'].entries()) {
      seedCommit(trailDb, `s${i}`, hash, 'Taro', 'packages/trail-activity/src/a.ts', RECENT);
    }
    const res = await fetch(`http://127.0.0.1:${port}/api/bus-factor`);
    ...
    expect(body.entries[0].unitId).toBe('packages/trail-activity/src/a.ts');
  ...
  it('minCommits 未満は score が null で返る', async () => {
    seedCommit(trailDb, 's1', 'h1', 'Taro', 'packages/trail-activity/src/a.ts', RECENT);
  ...
  it('windowDays より古いコミットは除外される', async () => {
    seedCommit(trailDb, 's1', 'h1', 'Taro', 'packages/trail-activity/src/a.ts', '2020-01-01T00:00:00.000Z');
