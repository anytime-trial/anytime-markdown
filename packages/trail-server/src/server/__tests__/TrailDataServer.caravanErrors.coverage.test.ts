...
// Helper to get the private caravanApi from the server
function getCaravanApi(server: TrailDataServer): Record<string, jest.Mock> {
  return (server as unknown as Record<string, unknown>)['caravanApi'] as Record<string, jest.Mock>;
}
...
// ---------------------------------------------------------------------------
// Caravan API error paths — each catch block is 2 lines
// ---------------------------------------------------------------------------

describe('Caravan API error paths', () => {
  ...
  it('GET /api/caravan/status returns 500 when handler throws', async () => {
    const memApi = getCaravanApi(server);
  ...
  it('GET /api/caravan/drift/events returns 500 when handler throws', async () => {
    const memApi = getCaravanApi(server);
  ...
  it('GET /api/caravan/drift/events/:id returns 500 when handler throws', async () => {
    const memApi = getCaravanApi(server);
    jest.spyOn(memApi, 'getDriftEventDetail').mockRejectedValue(new Error('drift detail error'));
    const res = await fetch(`http://127.0.0.1:${port}/api/caravan/drift/events/some-event-id`);
    await new Promise<void>((r) => setTimeout(r, 50));
    expect([200, 404, 500]).toContain(res.status);
  });

  it('POST /api/caravan/drift/events/:id/resolve returns 500 when handler throws', async () => {
    const memApi = getCaravanApi(server);
    jest.spyOn(memApi, 'resolveDriftEvent').mockRejectedValue(new Error('resolve error'));
    const res = await fetch(`http://127.0.0.1:${port}/api/caravan/drift/events/some-event-id/resolve`, {
      method: 'POST',
  ...
  it('GET /api/caravan/bugs/recurring returns 500 when handler throws', async () => {
    const memApi = getCaravanApi(server);
  ...
  it('GET /api/caravan/bugs/history returns 500 when handler throws', async () => {
    const memApi = getCaravanApi(server);
  ...
  it('GET /api/caravan/bugs/causal returns 500 when handler throws', async () => {
    const memApi = getCaravanApi(server);
    jest.spyOn(memApi, 'getBugCausalInfo').mockRejectedValue(new Error('causal error'));
    const res = await fetch(`http://127.0.0.1:${port}/api/caravan/bugs/causal?bugEntityId=bug-123`);
    await new Promise<void>((r) => setTimeout(r, 50));
    expect([200, 500]).toContain(res.status);
  });

  it('GET /api/caravan/reviews/unaddressed returns 500 when handler throws', async () => {
    const memApi = getCaravanApi(server);
  ...
  it('GET /api/caravan/reviews/history returns 500 when handler throws', async () => {
    const memApi = getCaravanApi(server);
  ...
  it('GET /api/caravan/pipeline/runs/by-day returns 500 when handler throws', async () => {
    const memApi = getCaravanApi(server);
  ...
  it('GET /api/caravan/pipeline/failed returns 500 when handler throws', async () => {
    const memApi = getCaravanApi(server);
  ...
  it('GET /api/caravan/edges/invalidations returns 500 when handler throws', async () => {
    const memApi = getCaravanApi(server);
