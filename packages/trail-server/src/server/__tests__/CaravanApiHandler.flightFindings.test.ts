 ...
 */
import { BetterSqlite3CaravanDb, runMigrations, type CaravanDbSqlValue } from '@anytime-markdown/trail-caravan-book';
...
function seedCaravanDb(dbPath: string): void {
  const db = new BetterSqlite3CaravanDb({ filePath: dbPath });
  runMigrations(db);
  const run = (sql: string, params: readonly CaravanDbSqlValue[] = []): void => {
    db.run(sql, params);
  };
 ...
 * なく caravanDbPath へ FlightRecordDatabase 経由でシードする（openInstruction がテーブルを
 * 冪等作成し、caravan_instruction_sessions へ起点セッションを紐付ける）。
 */
function seedInstructionSession(caravanDbPath: string): void {
...
describe('CaravanApiHandler flight review findings', () => {
  let tmpDir: string;
  let handler: CaravanApiHandler;

  beforeAll(async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'am-flight-findings-'));
    const caravanDbPath = path.join(tmpDir, 'caravan-book.db');
    seedCaravanDb(caravanDbPath);
    seedInstructionSession(caravanDbPath);
    handler = new CaravanApiHandler(makeMockLogger(), caravanDbPath);
  ...
  it('200 件を超える limit 指定を 200 へ丸めない', async () => {
    ...
    const caravanDbPath = path.join(manyDir, 'caravan-book.db');
    seedCaravanDb(caravanDbPath);
    seedInstructionSession(caravanDbPath);

    const db = new BetterSqlite3CaravanDb({ filePath: caravanDbPath });
    db.run(
      `INSERT INTO caravan_reviews (id, source_kind, source_ref, review_entity_id, target_kind, title, reviewer, reviewed_at, recorded_at, workspace)
    ...
    const h = new CaravanApiHandler(makeMockLogger(), caravanDbPath);
    try {
  ...
  it('activity.db が無ければ空を返しログへ理由を残す', async () => {
    ...
    const caravanDbPath = path.join(emptyDir, 'caravan-book.db');
    seedCaravanDb(caravanDbPath);
    const logger = makeMockLogger();
    const h = new CaravanApiHandler(logger, caravanDbPath);
    try {
