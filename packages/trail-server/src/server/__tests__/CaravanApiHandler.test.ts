jest.mock('@anytime-markdown/trail-caravan-book', () => {
  // Real BetterSqlite3CaravanDb は使う (better-sqlite3 への依存を mock しない)
...
describe('CaravanApiHandler', () => {
  ...
  describe('handleStatus', () => {
    ...
    it('returns exists:false when db file is absent', async () => {
      const h = new CaravanApiHandler(makeMockLogger(), path.join(tmpDir, 'no-such.db'));
