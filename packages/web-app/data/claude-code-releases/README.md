...
日次・週次が止まっていた区間は、**その区間を一次情報（公式 changelog・Anthropic のアナウンス）から直接調べ直した追補レポートを `<docsRoot>/report/` に置き、それを正本にする**（例: `20260922-claude-code-release-catchup.ja.md`）。年表に無いリリースは「そのリリースが無かった」ようにしか見えないため、レポート再開を待たない。`sourceReport` には必ず実在するレポートのファイル名を書く — 出典の無いエントリは後から真偽を確かめられない。
...
- `version`: **`kind: "model"` は系列名を含めて書く**（`Opus 4.7`。裸の `4.7` はスクリプトが例外で落とす）。年表には Opus / Sonnet / Fable / Mythos が併記されており、系列を機械が推測すると存在しない製品名を作ってしまう。どの系列かはレポートを読んだ人にしか判断できない
...
- 同じバージョンが複数レポートに現れてよい。統合は再生成側が行う。ただし**両方が `explicit` で日付が食い違うと警告が出る**（先に読んだ日付を採用する）。警告が出たらどちらが正しいか raw を直して解消する — テストが未解決の矛盾を落とす
- 週次まとめの範囲表記（`2.1.150-2.1.157`）は、同じ版の単独エントリ（`2.1.150`）とは別エントリとして残る。単発リリースと期間まとめは別の観測なので統合しない
...
- 各エントリの `highlights` は変更点の全量ではない。1 版で 100 項目を超える回もあり、抽出時に取捨選択している（2026-08-08 以降の追補分は、Claude Tag・Claude Code on the web・エンタープライズゲートウェイの細目を落とした）
