 ...
 * 「今どうなっているか」を読めなくなるため。
 *
 * 最新回は警告行ではなく `boundary_drift_runs`（警告 0 件でも積まれる）から
...
export function listBoundaryDriftDirect(
  ...
  if (!tableExists(db, 'boundary_drift_warnings') || !tableExists(db, 'boundary_drift_runs')) {
  ...
  if (latestOnly) {
    ...
    conditions.push(
      `EXISTS (SELECT 1 FROM boundary_drift_runs r
                WHERE r.repo_id = w.repo_id AND r.detected_at = w.detected_at
                  AND r.detected_at = (SELECT MAX(r2.detected_at) FROM boundary_drift_runs r2
  ...
  const rows = all<WarningRow>(
    ...
    `SELECT w.kind, w.target_key, w.stable_key, w.detected_at, w.severity, w.node_count,
       ...
       FROM boundary_drift_warnings w
       ${where}
...
function latestRuns(db: Database, repoId: number | undefined): BoundaryDriftRunSummary[] {
  ...
  return all<{
    ...
    db,
    `SELECT repos.repo_name AS repo_name, r.detected_at, r.warning_count, r.node_count
       FROM boundary_drift_runs r
       LEFT JOIN repos ON repos.repo_id = r.repo_id
       ${where}
       ${where === '' ? 'WHERE' : 'AND'} r.detected_at = (
         SELECT MAX(r2.detected_at) FROM boundary_drift_runs r2 WHERE r2.repo_id = r.repo_id
       )
