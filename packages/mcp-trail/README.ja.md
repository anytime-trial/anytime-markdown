...
| ツール | 主要パラメータ | 用途 |
| --- | --- | --- |
| `get_important_files` | `limit?`（既定 10）/ `filter?`（`central` / `dead` / `barrel` / `risky`、既定は総合重要度） | 重要度スコア上位のファイルを `{ rank, filePath, importanceScore, centralityScore, signals, reason }` で返す。discovery の起点（どこから読むか）を低トークンで決める |
| `get_code_dependencies` | `nodeId`（コードグラフのノード ID。多くはファイルパス。`list_community_nodes` / `get_important_files` から取得） | 指定ノードの依存元（incoming）・依存先（outgoing）を返す（depth 1）。変更前の影響範囲（blast radius）確認に使い、import の grep を畳む |
