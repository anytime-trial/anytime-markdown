...
- 書き込み系は `TrailDataServer` 起動時は HTTP 経由（in-memory 整合性維持）、未起動時は `sql.js` でファイル全体を読み込んで修正し、tmp + rename で atomic 書き出しします。
...
| `TRAIL_DB_PATH` | SQLite DB の絶対パスを直接指定（既定の解決ロジックをバイパス） |
| `TRAIL_WORKSPACE_PATH` | ワークスペースの絶対パス。`<workspace>/.vscode/trail.db` 解決と `repoName` 既定値（basename）に使用 |
| `MCP_TRAIL_FORCE_DIRECT` | `1` で `TrailDataServer` 生存チェックを skip し、書き込みも常に SQLite 直書きにする（CI / バッチ向け） |
| `MEMORY_CORE_DB_PATH` | memory-core SQLite DB の絶対パス（ドリフト・レビュー・バグ・メモリ検索ツール用） |
| `OLLAMA_BASE_URL` | Ollama API のベース URL（`search_memory` のベクトル検索で使用。例: `http://localhost:11434`） |

DB パス解決優先順位（最初に存在するファイルを採用）:

1. `TRAIL_DB_PATH`
2. `${TRAIL_WORKSPACE_PATH (or process.cwd)}/.vscode/trail.db`
3. `~/.claude/trail/trail.db`
4. `~/.vscode-server/data/User/globalStorage/anytime-trial.anytime-trail/trail.db`
...
memory-core DB（`MEMORY_CORE_DB_PATH`）を直接読み書きする。TrailDataServer は不要。
