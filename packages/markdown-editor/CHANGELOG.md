...
All notable changes to `@anytime-markdown/markdown-editor` will be documented in this file.
...
- Version bumped to stay in sync with the markdown release set. No functional change in this package.
...
- The embedded cooccurrence network diagram (markdown-rich-editor / graph-core) now supports directed links with arrowheads, a time axis of slices, and per-element notes. See the graph-core changelog for details.
...
- Cooccurrence network diagram (markdown-rich-editor / graph-core): a new anytime-graph diagram type showing which factor events are discussed together with a given subject, encoding frequency as circle area, co-occurrence strength as line width, and cluster membership as color.
...
- Screenmock fenced blocks (markdown-rich-editor): a `screenmock` code fence renders an interactive screen mockup with screen navigation, transitions and a component vocabulary. A design edit mode supports mouse resizing of parts and writes the edited layout back to the source.
...
- Render fenced `markdown` code blocks as a rendered preview (markdown-rich-editor).
...
- Stop leaving role/aria-label residue when toggling the preview (markdown-rich-editor).
...
- `markdown-rich-editor`: restored the syntax-highlight colors and layout of the code-block edit preview.
- `markdown-rich-editor`: show a live preview in the right pane of the HTML edit screen.
...
- `markdown-rich-editor`: added combo-stacked / combo-area / markers sample charts to the `anytime-chart` fence.
...
- Fixed a dark-mode regression in `markdown-rich-editor` where Mermaid previews rendered with light-mode colors.
...
- **Fully removed React from the editor core.** All NodeViews (footnote, gif, image, table) and the editor chrome (overlays, dialogs, settings panel, UI primitives) are now native/vanilla implementations with no React dependency; the editor and `markdown-rich-editor` packages are React-free.
...
- Inverted the `markdown-rich-editor` codeblock architecture to a native content NodeView with string/embed/math-graph previews and a full-screen edit dialog.
...
- Renamed the package from `@anytime-markdown/markdown-core` to `@anytime-markdown/markdown-editor`.
...
- Relocated the rich code-block cluster (diagrams, PDF dark-diagram rendering) to `@anytime-markdown/markdown-rich-editor`, removed those re-exports from the public API, and now inject the `codeBlock` extension via `getBaseExtensions`. A shared API is exposed for `markdown-rich-editor`.
