...
`@anytime-markdown/markdown-editor` に関するすべての注目すべき変更をこのファイルに記録します。
...
- `constants/colors.ts` に写しの所在を書いた。背景・浮き面・前景・補助文字・境界・強調・警告の 7 値（ライト／ダーク）は `diagram-viewer` の `DIAGRAM_THEME_TOKENS` に写しがある（viewer の配布バンドルがこのパッケージを import すると barrel ごとモジュールグラフを引き込むため）。パッケージをまたぐ一致は機械で測れないので、同時に直す必要のある値の隣に写しの所在を明記した。挙動は変わらない。
...
- i18n リソース（`en` / `ja`）にナビゲーション用の `diagramPage` ラベル（「系図」）を追加した。ビューア画面を一覧する側が系図の画面名を出せるようにするため。
...
- markdown リリースセットとバージョンを同期するための更新。本パッケージの機能変更はない。
...
- エディタの chrome / vanilla コンポーネント / 拡張にまたがる高複雑度関数を分割した（SonarCloud S3776 の 28 件）。挙動は変えていない。

### 修正

- data 属性の書き込みを `setAttribute` から `dataset` へ置き換えた（S7761）。
- 残る SonarCloud high 指摘を解消した（比較関数なしの `sort`＝S2871、冗長な `void`＝S3735、非推奨 DOM API＝S7768）。
...
- ランディング用の i18n に、ロードマップ要件 3 件（実行時呼び出しの追跡・判断品質の自己診断・構造ドリフトの可視化）の文言を追加しました。
...
- 埋め込みの共起ネットワーク図（markdown-rich-editor / graph-core 経由）が共起の向きの矢印・スライスによる時間軸・要素メモに対応した。詳細は graph-core の変更履歴を参照。

## [1.17.0] - 2026-07-22

### 追加

- ホストモード制御: `initialMode` オプションを追加しました。ホスト（web-app の `/markdown?mode=review` リンク等）が起動モードを明示的に指定でき、`defaultSourceMode` や localStorage の永続値より優先されます。初期適用は localStorage へ永続化しないため、レビューリンクで開いたタブがユーザーの通常起動モードを上書きしません。
- 共起ネットワーク図種（markdown-rich-editor / graph-core）: anytime-graph に、ある主題事象と一緒に語られる要因事象を一枚で読める共起ネットワーク図を追加しました。円の大きさ=出現頻度、線の太さ=共起強度、色=クラスタで符号化します。

### 修正

- 山括弧のオートリンク（`<https://...>`）が DOMPurify に不正な HTML タグとして扱われ `sanitizeMarkdown` で消失する問題を修正しました。既存の MATH/FN/ADM/CMT と同じ退避・復元パターンをオートリンク用に追加しています。
- 共起ネットワーク図種の追加により web-app の本番ビルド（`next build` の型チェック）が失敗する回帰を修正しました。`describeNode` の網羅性チェックが新図種に追従していませんでした。

## [1.16.0] - 2026-07-19

### 追加

- screenmock 編集パネル（markdown-rich-editor）: 編集ダイアログの右側に部品・属性・構造・画面のパネルを追加しました。バリアント・色トークン・オフセット入力・ドラッグ挿入・階層ツリー・画面の追加/複製/削除・id/title 編集に加え、スニペット・画面テンプレート・プリセット・並べ替え・整列・ズームに対応します。
- screenmock デザイン編集: ドラッグ並べ替えと Alt 自由配置、操作ヒントとドラッグバッジ文言を追加し、ドラッグ中は掴んだ要素がカーソルへ追従します。
- screenmock の書き戻し処理を専用の HTML 変異モジュールへ分離しました（挿入・削除・複製・文言・遷移先・クラス切替・style 宣言・オフセット・画面ブロック操作）。
- エディタ: Ctrl/Cmd + ホイールで本文テキストと図を一括拡縮できるようになりました。
- コードブロック編集ダイアログ: コードペインの折りたたみ/展開アイコンを追加しました。

### 修正

- 適用後に再オープンしたコードブロック編集ダイアログが古い本文を表示していました。
- ズーム表示中に screenmock のデザイン編集を操作できず、また要素ドラッグがプレビューのパンと二重に走っていました。
- 自由配置で absolute 座標へ切り替えず、`position: static` を `relative` へ読み替えることでフロー配置を維持するようにしました。
- screenmock の並べ替えをネイティブドラッグが奪う問題を抑止し、インデントを保つようにしました。
- screenmock 編集パネルをプレビューの下ではなく右側へ横並び配置しました。
- divider 表示の書き込み主体を `updateLayout` へ一元化しました。
- 相互レビュー指摘の対処: `sm-screen` 解除ガード・`pointercancel` 未処理・プリセット `custom` 表示・画像ソース対象を `img` に限定・サイズの独立書き戻し・複製の親解決・二重描画・bare ソースでの画面追加/複製・オフセット 0 の `position` 保持・id 重複拒否。

## [1.15.0] - 2026-07-17

### 追加

- screenmock フェンス（markdown-rich-editor）: `screenmock` コードフェンスが、画面遷移・トランジション・部品語彙を備えたインタラクティブな画面モックを描画します。デザイン編集モードでは部品のマウスリサイズに対応し、編集したレイアウトをソースへ書き戻します。
- スラッシュコマンド: vanilla エディタのスラッシュメニューに「screenmock」挿入項目を追加しました。

### 修正

- screenmock 機能の cross-review 指摘 9 件を修正しました（`:host` の CSS 変数解決・DOM 再接続後の再描画・再生成をまたぐ選択保持・`aria-label`・編集 UI 保護・`srcset` による画像ソース迂回・テーマ反映・プレビュー幅）。

### 追加

- セクションロック: 見出しセクションを「確定」としてロックできるようになりました（FR-8.3）。ロックされたセクションはエディタ内での編集が拒否され、アウトラインパネルに見出しごとのロック/解除ボタンが表示され、ロック状態は文書の frontmatter に永続化されます。共有パッケージ `section-lock-core` を新設し、エディタ・MCP サーバー・エージェントフックが同一ルールで検査します。

### 修正

- アウトラインパネルのロックボタンが表示されない・更新されないバグ 2 件を修正しました。
- セクションロックを含む文書で frontmatter が自動展開されるのを抑止しました。
- ロック付き文書のマウントがステータスバーの初期化順序バグで失敗するのを修正しました。
- 相互レビューの合意指摘 8 件に基づき、セクションロック実装全体を堅牢化しました。
...
- フェンス付き `markdown` コードブロックをレンダリングプレビュー表示（markdown-rich-editor）。
...
- プレビュー切替時に role/aria-label が残留しないよう修正（markdown-rich-editor）。
...
- markdown-rich-editor: PlantUML encoder を修正、KaTeX/印刷 SVG をサニタイズ、graph preview の i18n を完遂。
...
- `markdown-rich-editor`: コードブロック編集プレビューの構文ハイライト色とレイアウトを修復。
- `markdown-rich-editor`: HTML 編集画面の右ペインに実プレビューを表示。
...
- `markdown-rich-editor`: `anytime-chart` フェンスに combo-stacked / combo-area / markers のサンプルチャートを追加。
...
- `markdown-rich-editor`: mermaid プレビューがダークモードでライト色になる回帰を修正。
- `markdown-rich-editor`: 図ブロック編集ダイアログの白背景とタブ行レイアウト崩れをダークモードで修正。
...
- **エディタコアから React を完全排除。** 全 NodeView（footnote・gif・image・table）とエディタ chrome（オーバーレイ・Dialog・設定パネル・UI プリミティブ）を React 非依存の native/vanilla 実装へ移行。エディタ本体と `markdown-rich-editor` は React-free。
...
- `markdown-rich-editor` のコードブロックを反転アーキテクチャの native content NodeView 化（string/embed/math-graph プレビュー + 全画面編集ダイアログ）。
...
- パッケージ名を `@anytime-markdown/markdown-core` から `@anytime-markdown/markdown-editor` へ改名。
...
- リッチコードブロッククラスタ（図表・PDF ダーク図表レンダリング）を `@anytime-markdown/markdown-rich-editor` へ移設し、該当 re-export を公開 API から削除。`codeBlock` 拡張は `getBaseExtensions` で注入する。`markdown-rich-editor` 向けに共有 API を公開。
