...
- `importDrawio` の HTML タグ除去を index 走査に置き換えた。`/<[^>]*>/g` は閉じない `<` に対して残り全長ぶんバックトラックしていた（Sonar S8786）。
- グラフ DSL パーサの箇条書き・インデントツリー・共起リンク・`cluster` 行で、ラベル境界を `\S` で固定した。非貪欲量指定子と `\s*` の重なりによる super-linear を解消した。
- `sha256` のコードポイント読み取りを `codePointAt` へ置き換えた。対になっていないサロゲートは従来どおり U+FFFD へ写す。

## [1.3.0] - 2026-08-20

### 変更

- graph リリースセットとバージョンを同期するための更新。本パッケージの機能変更はない。
...
- `CooccurrenceFile.layout` に任意の `source` フィールド（`'client'` | `'server'`）を追加した。サーバ供給の座標は供給元が正しさに責任を持つため、アルゴリズム版数を照合せずそのまま使う。版数照合はクライアントのキャッシュ経路だけに残す。
- `layout` の構造検査を追加した（座標は `[x, y]` の組であること）。

## [1.0.1] - 2026-08-03

### 変更

- テストのみ: `barnesHutLayout` の指標判定をアサーション経由へ戻した。挙動の変更はない。

## [1.0.0] - 2026-08-01

### 追加

- 共起の語と結びつきを 1 回の呼び出しで足す編集 API を追加した。
- クラスタの中をもう 1 段細分するサブクラスタを追加し、絞り込みの対象にした。

### 変更

- ミニマップの定数を 3 つの実装で共有するようにした。
- Web Component の SSR 安全化基底と translator の解決を `@anytime-markdown/ui-core` へ移した。

### 修正

- 空の名前を持つサブクラスタを検証で拒否するようにした。
