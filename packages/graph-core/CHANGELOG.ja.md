...
- 共起の向き: 型と読み書き関数、DSL 記法、編集関数、静的図の矢印、公開エクスポート。向きを実際に使ったときだけ版数を 2 へ繰り上げ、`specHash` からは向きを外して既存ファイルのハッシュを保つ。
- 共起ネットワークの時間軸: ラベル付きスライスを読み書きで受け渡し、レイヤーとして並置して同一語を点線で結ぶ。
- 語・共起・クラスタへのメモ。

### 変更

- 絞り込みとレイアウトを `readLink` 経由で読むようにした。

### 修正

- 自己共起・範囲外の端点・負値の検証が、向き付きの共起にも効くようにした。従来は向き付きの経路で検証が丸ごと飛ばされ、UI からも MCP からも不正なファイルを書けた。

## [0.12.0] - 2026-07-22

### 追加

- 思考法ダイアグラムの 12 番目として共起ネットワークを追加。円の大きさ=出現頻度（面積比例・半径は sqrt スケール）、線の太さ=共起強度、色=クラスタで符号化。頻度・強度は DSL に明示的に書く方式で、本文テキストからの自動抽出は行わない。
- Web アプリ・VS Code 拡張・MCP ツールが共有する `.cooc.json` ファイル形式とスキーマ検証を追加。検証は誤りを 1 件目で打ち切らず全件返し、座標キャッシュの無効化はレイアウトアルゴリズムの版数で判定する（spec ハッシュだけでは判定しない）ため、レイアウト改良後の既存ファイルが旧座標のまま描画される事故を防ぐ。
- 共起ネットワーク向けの Barnes-Hut レイアウト（斥力計算を四分木で近似）を追加。2,000 語のレイアウト時間を 8,549ms から 336ms へ短縮。語数に応じてスケールする求心力で、語数が増えても図が発散しないようにした。
- 共起ネットワークの絞り込み（最小出現頻度・クラスタ選択・最小強度・上位 N 本）と、全ノード添字参照を一貫して付け替える編集操作（語の追加・削除・改名）を追加。
- 共起ネットワークの描画コア（canvas 描画・ラベル配置・ヒットテスト・ビューポート/ズーム・テーマ同期）、フィルタパネルと仮想化語一覧パネル、専用レイアウト Worker、ja/en の i18n、Web アプリの `/cooccurrence` ページ（`cooccurrence-viewer` パッケージ）を追加。

### 変更

- 共起ネットワークビューアの描画を毎フレームではなく要求時のみへ変更。アイドル時のテーマ変数の継続読み取り・canvas バッキングストアの再確保・O(n²) のラベル重なり判定を削減。

### 修正

- フォースレイアウトで孤立語（共起を持たない語）が無制限に発散する不具合を修正。修正前は原点から 13,000px 超まで拡散し、viewBox の関係で他の語が点に潰れて見えなくなっていた。
- 入力検証の漏れを修正。重複した語定義と自己共起（`A -- A`）をエラーとして拒否し、頻度・強度の負値も最小サイズの円へ黙って丸めず拒否するようにした。
- `computeSpecHash` が Node の `node:crypto` に依存しており、ブラウザ向けバンドル（VS Code 拡張の webview を含む）のビルドが落ちる問題を修正。同一の出力を返す環境非依存の純 JS SHA-256 実装へ置き換えたため、既存 `.cooc.json` の `layout.specHash` はそのまま有効。
- 全クラスタ選択中でもクラスタ未所属の語が図から消えていた絞り込みの不具合を修正。
- 共起ネットワークビューアの不具合を修正: レイアウト Worker を編集のたびに終了させず蓄積させていた問題、中断したレイアウトの Promise が pending のまま残る問題、レイアウト失敗とユーザーによる中断が同じ状態として扱われていた問題、Blob URL の競合で書き出しファイルが空に読める問題、日本語などの非 ASCII タイトルが書き出しファイル名から失われる問題、PNG 書き出しの payload が未検証だった問題。
- 語一覧パネルが再描画のたびにキーボードフォーカスを失う問題を修正し、仮想化した行に `aria-setsize`/`aria-posinset` を付与した。
- 右サイドパネル（フィルタ・語一覧）の下部が切れて到達不能だった問題を修正。パネル列自体を独立したスクロール領域にした。
