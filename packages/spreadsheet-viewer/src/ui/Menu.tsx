...
function axisPercent(value: "top" | "center" | "bottom" | "left" | "center" | "right"): string {
...
function anchorPoint(el: HTMLElement, origin: MenuOrigin): MenuPosition {
  ...
  const left = origin.horizontal === "left" ? r.left : origin.horizontal === "right" ? r.right : r.left + r.width / 2;
  const top = origin.vertical === "top" ? r.top : origin.vertical === "bottom" ? r.bottom : r.top + r.height / 2;
  return { top, left };
}

/**
 * MUI Menu の置換。anchorEl 基準（SheetTabs）と anchorPosition 絶対座標基準（ContextMenu）の両対応。
...
export function Menu({
  ...
  return createPortal(
    <>
      <div className="sv-menu-backdrop" onMouseDown={onClose} />
      <div
        ref={paperRef}
        className="sv-menu-paper"
        role="menu"
        tabIndex={-1}
