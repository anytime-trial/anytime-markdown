...
`anytime-reverse-codegraph` でコミュニティに `name` / `summary` / `mappings_json` が付与済みであることを前提に、`{outputDir}` 配下に基本設計書 8 章（システム概要 / 機能一覧 / 機能詳細 / データモデル / 外部 I/F / 画面仕様 / データフロー / 用語集）を生成する。任意のリポジトリに適用可能。
...
| 5 | `/anytime-reverse-spec chapter=all mode=wash-away`（初回） | 全ファイル生成（章 0〜8 の 9 ファイル）、`validate-markdown.sh` 全 OK、`_community-map.json` 整合 |
