...
> Step 2 / Step 3 の DB アクセスはすべて MCP ツール経由（読み取りは `list_communities` / `list_community_nodes`、書き込みは `upsert_community_summaries` / `upsert_community_mappings`）。Trail Viewer は Reload Window なしに反映される（拡張側 `model-updated` 通知）。\
