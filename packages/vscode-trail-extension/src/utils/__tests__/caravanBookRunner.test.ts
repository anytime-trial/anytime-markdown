...
// jest 環境で初期化エラーになるため、runtime/caravanBookRunner を直接 import する。
import { createCaravanBookRunner } from '../../../../trail-server/src/runtime/caravanBookRunner';

// CaravanBookService コンストラクタの呼び出しを観測するための spy。
const runOnceMock = jest.fn();
const constructorSpy = jest.fn();

jest.mock('@anytime-markdown/trail-caravan-book/pipeline', () => {
  return {
    CaravanBookService: jest.fn().mockImplementation((opts: unknown) => {
      constructorSpy(opts);
      return {
        runOnce: runOnceMock,
      };
...
describe('createCaravanBookRunner.runAfterImport', () => {
  test('runAfterImport delegates to CaravanBookService.runOnce("import")', async () => {
    const channel = makeChannel();
    const runner = createCaravanBookRunner({
    ...
    // CaravanBookService が 1 度だけ生成される (lazy 構築)
  ...
  test('subsequent runAfterImport reuses the same CaravanBookService instance', async () => {
    const channel = makeChannel();
    const runner = createCaravanBookRunner({
      outputChannel: channel,
      trailDbPath: TRAIL_DB_PATH,
  ...
  test('passes nativeBinding and gitRoot through to CaravanBookService', async () => {
    const channel = makeChannel();
    const runner = createCaravanBookRunner({
      outputChannel: channel,
      trailDbPath: TRAIL_DB_PATH,
