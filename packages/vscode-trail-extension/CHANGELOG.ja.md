...
- 同梱スキル `anytime-dev-retro` で MCP サーバーの利用消失（`mcpHealth`）を測るようにした。サーバーごとの直近 30 日 / 前 30 日の呼出数、編集ターンがあるのに呼出ゼロのサーバー、呼出が消えたサーバーを出す。あわせて Flight Record の最終記録時刻を出し、7 日超前ならレビュー系指標を測定不能として扱う。

### 修正

- stop-hook と緊急時の spool drain が、監視対象の git ルート全部（設定のワークスペースパス・ワークスペースフォルダ・`lep.json` の gitRoots を git common dir 単位で重複排除）を処理するようにした。これまではワークスペースパスが存在しないディレクトリを指すと drain が黙って 0 件を返し、フックの spool が未消化のまま Flight Record が欠落していた。解決できないパスは 1 回だけ警告する。

### Trail Core (trail-db)

- Supabase 同期で PostgREST の接続系エラー（`PGRST0xx`。例: PGRST002）を恒久エラー扱いせず再試行するようにし、洗い替えの全削除でエラーを無視して upsert へ進んでいたのを直した。
...
- MCP 登録コマンド: `.mcp.json` の登録を「無ければ書く」から鮮度検証つきの再登録（reconcile）へ変更。旧版が書いた古いエントリが更新されるようにした。
- MCP 登録コマンド: reconcile の更新を全置換から部分更新へ変更。既存エントリに利用者が足したフィールド（`cwd` 等）が消えないようにした。
...
- 知識グラフ画面に検索と ego 表示を追加した。識別子や概念を入力すると、該当ノードを近傍ごと中心に表示する。
- エージェントの照会（`search_caravan_book`）の結果を知識グラフ上でハイライト・選択表示するようにした。エージェントが何を読んだかが画面で分かる。
- コミュニティ要約を段階的に付与し、検索結果とクラスタ表示へ統合した。
- 実行記録の 3 タブ（Bug Fixed / Review / Drift）へワークスペース列を追加し、知識グラフのカード表示で縦の枚数を選べるようにした。

### 変更

- トップレベルタブの順序を アクティビティ / 実行記録 / C4モデル / 知識グラフ / パイプライン / トレース / 関数ツリー / Chat へ変更した。

### Trail Core (trail-activity / trail-server / trail-viewer / trail-caravan-book / mcp-trail)

- 因果カード取得 `get_bug_causality` と計画コンテキストパック `get_plan_context` を追加した。`search_caravan_book` に段階開示・エッジ集約・abstain を加えた。
- レビュー指摘の対処率を分母別に集計し、抽出経路を指摘へ刻印（`extracted_by`）するようにした。裸のファイル名は一意なパスへ解決して補完し、対処コミットと突き合わせられるようにした。
- コード由来の辺を `calls` / `extends` の述語へ分離し、検索評価ハーネス（golden task・1k トークンあたりの回収事実数）を追加した。

### 修正

- 消滅したファイルを指す drift を自動解決するようにした。再検出が人手の解決を無言で取り消すこともなくなり、再発として記録される。
- `search_caravan_book` が壊れた経路で応答していた問題を修理し、本来のハイブリッド検索へ切り替えた。
- git の引数順の誤りで反転していた SZZ 推定と、書込先テーブルを取り違えていた read-only attach ガードを修正した。
- 会話取込で無名・低情報のエンティティが生成されないようにした。

## [1.0.0] - 2026-08-09

### 追加

- 知識グラフを 100,000 ノード規模へ対応させた。レイアウト座標をサーバ側で事前計算し、全件ではなく視野単位で配信する。事前計算は解析デーモンの定期実行から起動する。

### 変更

- タブ表示名を変更した（Flight Record → CaravanBook / 実行記録、Trail Pipeline → パイプライン、C4 Model → C4モデル）。
- `memory` 用語を `caravan` / `caravan-book` へ一掃リネームし、3 つのデータベースのテーブルへ `activity_` / `caravan_` / `catalog_` の接頭辞を付けた。既存 DB は所有者（拡張・デーモン）が初めて開いた時点で移行される。

### 修正

- 知識グラフでハブノードが孤立点として描かれる問題を解消した（ノード選定をエッジ誘導化し、`File` → `Function` の `defines` 辺を出力する）。
- 配信対象の視野外を指すゴースト端点を除外した。
- 視野がデータの無い場所へ移っても図を消さないようにした。

### Trail Core (trail-activity / trail-server / trail-viewer / mcp-trail)

- 視野単位の知識グラフ配信 API を追加し、索引駆動クエリ（リンク取得は結合順を固定した CROSS JOIN）へ切り替えた。
- サーバ側でレイアウトを事前計算し、座標へ `source: 'server'` を付与する。
- 座標が未計算・書き替え中の窓で配信が壊れないようにした。
...
- Flight Record が「運航記録を取得できません（Trail サーバー停止の可能性）」としか表示されなくなる不具合を修正しました。0.43.0 で Flight Record の保存先を memory-core.db へ移した際、その接続だけがネイティブモジュールのパスを明示せずに開いており、バンドル済みの拡張では初期化に必ず失敗していました。サーバーは稼働しており、Flight Record 系の API だけが応答できない状態でした。運航記録の記録側（セッション終了時の取り込み）も同じ理由で失敗しており、0.43.0 の稼働中に終了したセッションは記録されていません。

### Trail Core (trail-core / trail-server / trail-viewer)

- SQLite 接続を開く経路を 1 箇所へ集約し、バンドル済みネイティブモジュールのパス解決を各クラスへ書き写さない構造に変えました。配置漏れを検知したときは警告を残します。

## [0.43.0] - 2026-08-08

### 追加

- Flight Record タブを新設しました。記録の単位をセッションから「指示」へ変え、1 つの依頼に複数セッションがぶら下がる実態をそのまま 1 行で追えるようにしました。指示ごとにレビュー指摘・Bug Fixed・Drift・検証の実施状況を並べて確認できます。

### 変更

- 同梱スキル `anytime-dev-retro` を v19 へ更新しました（Flight Record メトリクス、衛生行動の 3 期間平均、ドクトリンの指示不足を切り分ける採掘クエリ、token-budget の観測値を追加）。
- 同梱スキル `anytime-trail-review` を v5 へ更新しました。
- Memory タブを Trail Pipeline へ改称し、Drift サブタブを Flight Record 側へ移しました。タブ名が中身（取込パイプラインの状態）と合っていませんでした。

### 削除

- Logs タブを削除しました。拡張ログ専用の DB（`extension-logs.db`）を廃し、ログは実行台帳（`pipeline_run_logs`）へ集約したため、単独の閲覧画面は読み手のいない経路になっていました。
- ドキュメント検索 DB（doc-core）の ingest 配線を拡張から外しました。

### Trail Core (trail-core / trail-server / trail-viewer / memory-core / trail-db / mcp-trail)

- Flight Review を指示単位の Flight Record へ再設計し、関連 3 テーブルを `trail.db` から `memory-core.db` へ移設しました。
- ドクトリン判断を D2（低重大度・高可逆な What 承認の代行）へ昇格し、代行の記録と抜き取り監査の経路を追加しました。あわせて未確定論点の事前申告（DCT-14）で、較正の失敗と指示不足を別々に数えられるようにしました。
- ODD ポリシーレジストリと承認ルールエンジンを追加し、「常時人へ聞く操作」の判定を散文の遵守からカバレッジゲートへ移しました。
- PR レビューを `memory_reviews` へ統合し、レビュー記録にワークスペースと対象リポジトリを持たせて照合できるようにしました。Review 一覧には重大度・カテゴリ・状態のフィルタを追加し、対処状態を 3 値化して「未対処」の誤読を断ちました。
- 会話からの LLM 抽出で、取込対象を人間の入力に絞り、捏造・index 衝突・刻印漏れを塞いで失敗率を下げました。
- `pipeline_runs` を LEP 全 Wave の実行台帳へ一般化し、Runs 画面に Wave フィルタと失敗理由への導線、run 単位のログ参照 API を追加しました。
- 検証実施台帳を `trail.db` へ移し、Flight Record の指示タブに実施状況を表示するようにしました。
- 生の `<select>` を共通部品（ui-core Select）へ移行しました。ネイティブの popup が OS 既定配色になり、ダークテーマで判読できなくなっていました。

## [0.42.0] - 2026-08-04

### 追加

- 整合性ツリーと診断で、判定不能（unknown）を stale と別枠で表示するようにしました。

### 修正

- 整合性判定のカバレッジ基準を取込の実行時刻に改め、設計書リポジトリのコミットが未取込のケースを stale と誤判定しないようにしました。

### Trail Core (trail-core / trail-server / trail-viewer)

- コードグラフタブに Time Scrubber と Auto Playback を追加し、コミット粒度で State Replay を再生できるようにしました。
- コミット時点のコードグラフを保持するテーブルと生成 API を追加し、release タグからの遡及生成にも対応しました。
- コードグラフタブに前版との差分表示と、最終編集者・編集頻度による着色（`GET /api/author-heatmap`）を追加しました。
- git を PATH 探索でなく絶対パスで起動するよう全経路を移行し、退行を防ぐ CI ゲートを追加しました（SonarCloud S4036）。
...
- Phase 5 緊急プロトコル: Kill Switch・セーフポイント・非破壊ロールバックのコマンド、ビューアのフローティング Kill Switch ボタンと EmergencyPanel、緊急状態の取得・発動・解除・ロールバックの HTTP API を追加しました。
- ナレッジベース永続性: グラフ系破壊的書込の Pre-write Snapshot と Shrink Audit、縮小警告通知と復元コマンドを追加しました。
- サーバー不達時にエージェントフックが書き出す emergency spool の定期 drain を追加しました。
- 品質メトリクス: ビューアに MTTR / TCR カードを追加しました。
- ノード右クリックメニューに「Agent Note に出力」を追加しました（新設の WS コマンド / IPC イベント経由で Anytime Agent 拡張へ委譲）。

### 修正

- Kill Switch: 理由入力のキャンセルが記録なしの無言中断になるのを是正しました（FR-S5-5 / FR-S5-6）。
- `trail.db` が 2 GiB を超えると拡張が起動不能になる崖を除去しました（バックアップ・保存経路でファイル全体をメモリへ読み込まない）。
- trail-server / trail-viewer / trail-db にわたる相互レビュー指摘を是正しました。

### セキュリティ

- ローカル trail サーバーの永続書込 POST 全エンドポイントへ CSRF Content-Type ガードを拡張しました。

## [0.35.0] - 2026-07-16

### 変更

- 同梱スキル `anytime-dev-health` を `anytime-dev-retro` に改名し、ふりかえり（retrospective）に特化させた。環境・セットアップ監査は `anytime-dev-audit` として分離し、Anytime Agent 拡張側の同梱へ移管した。
- `anytime-dev-retro` にインシデントモード（事故の要件化）、プロンプト実測フィードバックループ（スキル発火の 2 窓比較・委任成績集計）、モデル別挙動プロファイルと委任記録のモデルタグ、メタ機構の健全性点検とメタ還流昇格（改訂が繰り返し効かない対象は還流機構側の改訂を提案）を追加した。
- 同梱スキル `anytime-token-budget` の本文を更新（manifest バンプで配布に反映）。
- スキルスクリプトの `.test.cjs` テストをパッケージの jest 実行に配線した（従来は未配線だった）。

### 修正

- 委任成績集計が日本語直後の採用/差し戻しマーカーを取りこぼしていた（JS の `\b` が日本語直後で不成立）。またモデル統計が短縮別名とフル ID にキー分裂しないようにした。
...
- Supabase 同期の FK 違反を修正しました（参照整合ゲート・一過性 HTTP 失敗のリトライ・チャンク単位のエラー隔離）。
...
- 同梱の anytime-dev-health スキルを更新: コストシグナルを直近30日ローリングウィンドウ化し、累積指標による偽陽性を是正。
