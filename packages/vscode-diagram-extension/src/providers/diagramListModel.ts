...
/** 系図ファイルの検索パターン（`vscode.workspace.findFiles` の include）。 */
export const DIAGRAM_FILE_GLOB = '**/*.diagram.json';

/** 検索から除外するパターン（`vscode.workspace.findFiles` の exclude）。 */
export const DIAGRAM_FILE_EXCLUDE_GLOB = '**/{node_modules,.git,out,dist}/**';
...
// 数値を数値として比較する（chart2 を chart10 より先に出す）。ロケール差で並びが揺れないよう
// 'en' に固定する。
const COLLATOR = new Intl.Collator('en', { numeric: true, sensitivity: 'variant' });

/**
 * 区切り文字を `/` に揃え、先頭の `./` を落とす。表示できる中身が残らなければ null を返す。
 */
export function normalizeDiagramRelativePath(raw: string): string | null {
 ...
 * ディレクトリ順 → ファイル名順に並べる。
 */
export function buildDiagramListEntries(relativePaths: readonly string[]): DiagramListEntry[] {
	const seen = new Set<string>();
	const entries: DiagramListEntry[] = [];

	for (const raw of relativePaths) {
		const relativePath = normalizeDiagramRelativePath(raw);
