...
function getChannel(): vscode.OutputChannel {
	if (_channel === undefined) {
		_channel = vscode.window.createOutputChannel('Anytime Diagram');
...
export const DiagramLogger = {
	/**
	 * activate() で生成済みの OutputChannel を共有する。
	 *
	 * init 前にログを書くと遅延生成が走る。そのチャンネルは context.subscriptions に入らず
	 * 破棄されないうえ、activate 側の同名チャンネルと 2 系統に分かれて「見ているチャンネルに
	 * 目的の行が無い」状態になるため、ここで捨てる。
	 */
