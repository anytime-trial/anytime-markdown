...
import { McpDiagramServerProvider } from '../McpDiagramServerProvider';
...
function stdioDefinitions(provider: McpDiagramServerProvider): StdioDefinition[] {
	return provider.provideMcpServerDefinitions(token) as unknown as StdioDefinition[];
}

describe('McpDiagramServerProvider', () => {
	afterEach(() => {
		mockVscode.workspace.workspaceFolders = undefined;
	});

	test('同梱バンドル (dist/mcp-diagram-server.js) を node で起動する定義を返す', () => {
		mockVscode.workspace.workspaceFolders = [{ uri: { fsPath: '/ws/project' } }];
		const provider = new McpDiagramServerProvider(DIST, '9.9.9');
		const [definition] = stdioDefinitions(provider);
		expect(definition.label).toBe('mcp-diagram');
		expect(definition.command).toBe(process.execPath);
		expect(definition.args).toEqual([path.join(DIST, 'mcp-diagram-server.js')]);
		provider.dispose();
	});

	test('ワークスペースを ANYTIME_DIAGRAM_ROOT で渡す（渡さないと cwd が拡張ホストになる）', () => {
		mockVscode.workspace.workspaceFolders = [{ uri: { fsPath: '/ws/project' } }];
		const provider = new McpDiagramServerProvider(DIST, '9.9.9');
		const [definition] = stdioDefinitions(provider);
		expect(definition.env).toEqual({ ANYTIME_DIAGRAM_ROOT: '/ws/project' });
		provider.dispose();
	});
	...
	test('version は呼び出し側から受け取る（package.json との二重管理を避ける）', () => {
		mockVscode.workspace.workspaceFolders = [{ uri: { fsPath: '/ws/project' } }];
		const provider = new McpDiagramServerProvider(DIST, '9.9.9');
		expect(stdioDefinitions(provider)[0]?.version).toBe('9.9.9');
		provider.dispose();
	});
