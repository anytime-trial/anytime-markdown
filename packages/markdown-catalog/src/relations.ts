/**
 * 型付きノート関係の語彙（markdown-catalog ローカルミラー）。
 *
 * 語彙の単一の真実は graph-core の `presets/relationStyle.ts`。ただし markdown-catalog は他パッケージ
...
export function coerceRelationType(value: unknown): RelationType {
  ...
  if (value !== undefined && value !== null && value !== '') {
    console.warn(
      `[markdown-catalog] unknown relation type ${JSON.stringify(value)}; falling back to '${DEFAULT_RELATION_TYPE}'`,
    );
  }
