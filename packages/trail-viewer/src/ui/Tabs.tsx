...
export interface TabsProps {
  ...
  readonly value: string | number | false;
  /** MUI 互換: (event, value) => void シグネチャ。 */
  readonly onChange: (event: SyntheticEvent, value: string | number) => void;
...
export function Tabs({ value, onChange, style, className, children, sx }: Readonly<TabsProps>) {
  injectTrailUiStyles();
    ...
    <div className={classes} role="tablist" style={{ ...sxToStyle(sx), ...style }}>
      {Children.map(children, (child) => {
        ...
        return cloneElement(el, {
          ...
          onSelect: (v: string | number, e: SyntheticEvent) => onChange(e, v),
        });
      })}
    </div>
  );
