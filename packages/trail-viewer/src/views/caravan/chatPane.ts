...
import {
  ...
  type ChatState,
  type ChatUiSource,
} from '../../components/caravan/chatReducer';
...
export function mountChatPane(
  ...
  const textField = createTextField({
    size: 'small',
    placeholder: props.t('caravan.chat.placeholder'),
    value: '',
    multiline: true,
    maxRows: 5,
  });
  ...
  const sendBtn = createIconButton({
    size: 'small',
    ariaLabel: props.t('caravan.chat.send'),
  ...
  const abortBtn = createIconButton({
    size: 'small',
    ariaLabel: props.t('caravan.chat.abort'),
  ...
  return {
    update(next) {
      ...
      // Update placeholder
      textInput.setAttribute('aria-label', next.t('caravan.chat.placeholder'));
      textInput.setAttribute('placeholder', next.t('caravan.chat.placeholder'));
    },
    destroy() {
      unsubscribe?.();
      unsubscribe = null;
