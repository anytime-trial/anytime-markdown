...
import type { ChatUiSource } from '../../components/caravan/chatReducer';
...
export function mountSourcesPanel(
  ...
  function render(): void {
    ...
    heading.textContent = props.t('caravan.chat.sources.title');
    root.appendChild(heading);

    if (props.sources.length === 0) {
      const empty = document.createElement('span');
      empty.style.cssText = 'font-size:0.75rem;color:var(--am-color-text-secondary);';
      empty.textContent = props.t('caravan.chat.sources.empty');
