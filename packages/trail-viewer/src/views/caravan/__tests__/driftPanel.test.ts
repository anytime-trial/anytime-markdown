/**
 * views/caravan/driftPanel — vanilla DOM ユニットテスト（jsdom）
 *
...
function makeRow(over: Partial<CaravanDriftEventRow> = {}): CaravanDriftEventRow {
  return {
    id: 'row-1',
...
describe('mountFiltersPanel', () => {
  ...
  it('タイトルと2つのラジオボタンを描画する', () => {
    ...
    expect(c.textContent).toContain('caravan.chat.filters.title');
