...
function makeReader(overrides: Partial<CaravanReader> = {}): CaravanReader {
  return {
    listRecurringBugs: () => Promise.resolve([] as readonly CaravanRecurringBugRow[]),
    getBugHistory: () => Promise.resolve([] as readonly CaravanBugHistoryRow[]),
    getBugCausalInfo: () => Promise.resolve(null),
    listUnaddressedReviewFindings: () =>
      Promise.resolve([] as readonly CaravanUnaddressedReviewFindingRow[]),
    getReviewHistory: () => Promise.resolve([] as readonly CaravanReviewHistoryRow[]),
    listPipelineRunStatsByDay: () =>
      Promise.resolve([] as readonly CaravanPipelineRunStatsByDayRow[]),
    listPipelineRuns: () => Promise.resolve([] as readonly CaravanPipelineRunRow[]),
    listPipelineRunLogs: () => Promise.resolve([] as readonly CaravanPipelineRunLogRow[]),
    listInvalidations: () => Promise.resolve([] as readonly CaravanInvalidationRow[]),
    listFailedItems: () => Promise.resolve([] as readonly CaravanFailedItemRow[]),
    ...overrides,
...
describe('mountBugHistoryPanel — Fix A: destroy guard', () => {
  it('late-resolving load() does not mutate container after destroy()', async () => {
    const { promise, resolve } = deferred<readonly CaravanBugHistoryRow[]>();
    const reader = makeReader({
      getBugHistory: () => promise,
      listRecurringBugs: () => Promise.resolve([]),
    } as Partial<CaravanReader>);
...
describe('mountBugHistoryPanel — ワークスペース切替の世代ガード', () => {
  ...
  it('切替前のワークスペースの応答が遅れて届いても表示に反映しない', async () => {
    const slowA = deferred<readonly CaravanBugHistoryRow[]>();
    const reader = makeReader({
      listRecurringBugs: () => Promise.resolve([]),
...
describe('mountPipelineRunsPanel — Fix A: destroy guard', () => {
  it('late-resolving loadData() does not mutate container after destroy()', async () => {
    const { promise, resolve } = deferred<readonly CaravanPipelineRunStatsByDayRow[]>();
    const reader = makeReader({
  ...
  it('stale loadData() response from old reader is ignored after update()', async () => {
    const { promise: stalePromise, resolve: resolveStale } =
      deferred<readonly CaravanPipelineRunStatsByDayRow[]>();
