/**
 * views/caravan/caravanPanel — vanilla DOM ユニットテスト（jsdom）
 *
 * - probe の 3 状態（loading / noDb / 本体）を出し分ける
 * - サブタブバーを持たない（2026-08-05 に畳んだ。Runs 単独のため）
 * - update() / destroy() が正しく動作する
 * - CaravanReader.probe() は no-throw（jsdom: no real server）
 */
import { mountCaravanPanel, type CaravanPanelViewProps } from '../caravanPanel';
...
function baseProps(over: Partial<CaravanPanelViewProps> = {}): CaravanPanelViewProps {
  return {
...
describe('mountCaravanPanel', () => {
  ...
  it('probe が false を返したら noDb メッセージを表示する', async () => {
    ...
    expect(c.textContent).toContain('caravan.noDb');
    expect(c.querySelector('[role="tablist"]')).toBeNull();
  });
  ...
  it('destroy() でルート要素を除去する', async () => {
    const c = document.createElement('div');
    const handle = mountCaravanPanel(c, baseProps());
    await flush(8);
  ...
  it('update() で t() が反映される（loading 中）', () => {
    const c = document.createElement('div');
    const handle = mountCaravanPanel(c, baseProps({ t: (_k) => 'ORIGINAL' }));
...
describe('mountCaravanPanel – probe が true のとき', () => {
  beforeEach(() => {
    const { CaravanReader } = require('../../../data/readers/CaravanReader') as {
      CaravanReader: { prototype: { probe: () => Promise<boolean> } };
    };
    CaravanReader.prototype.probe = async () => true;
  });

  afterEach(() => {
    const { CaravanReader } = require('../../../data/readers/CaravanReader') as {
      CaravanReader: { prototype: Record<string, unknown> };
    };
    delete CaravanReader.prototype.probe;
    document.body.querySelectorAll('[data-am-dialog-backdrop]').forEach((el) => el.remove());
  });
  ...
  it('update() の t() が本体にも反映される', async () => {
    const c = document.createElement('div');
    const handle = mountCaravanPanel(c, baseProps({ t: (_k) => 'OLD' }));
    await flush(8);
