/**
 * CaravanPanel（表示名 Trail Pipeline）の vanilla DOM 版。
 *
 ...
 *
 * したがって本パネルが持つのは `CaravanReader` と DB 存在プローブ（loading / noDb / 本体の
 * 3 状態）だけで、サブタブ状態・hash routing・サブビューの出し分けは持たない。
 * `#memory/<tab>` の hash も解釈しない（対応する選択肢が無い）。
 *
 * 呼び出し側（components/CaravanPanel.tsx）は thin React wrapper として
...
export interface CaravanPanelViewProps {
...
export function mountCaravanPanel(
  container: HTMLElement,
  initial: CaravanPanelViewProps,
): VanillaViewHandle<CaravanPanelViewProps> {
  let props = initial;
  let destroyed = false;
  ...
  const reader = new CaravanReader(props.serverUrl);
  ...
  loadingText.textContent = props.t('caravan.loading');
