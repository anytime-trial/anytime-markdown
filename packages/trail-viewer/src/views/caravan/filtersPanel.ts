/**
 * FiltersPanel の vanilla 版（`components/caravan/FiltersPanel.tsx` の素 DOM 等価）。
 *
...
export function mountFiltersPanel(
  ...
  title.textContent = props.t('caravan.chat.filters.title');

  const allLabel = createFormControlLabel({
    label: props.t('caravan.chat.filters.allRepos'),
    control: createRadio({ size: 'small' }),
    value: 'all',
  });

  const currentLabel = createFormControlLabel({
    label: props.t('caravan.chat.filters.currentRepo'),
    control: createRadio({ size: 'small' }),
    value: 'current',
  });
  ...
  return {
    update(next) {
      ...
      title.textContent = next.t('caravan.chat.filters.title');
    },
    destroy() {
      radioGroup.destroy();
      root.remove();
