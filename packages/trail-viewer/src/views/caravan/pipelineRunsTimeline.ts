...
import { buildStackedChartData } from '../../components/caravan/pipelineChartData';
...
export interface PipelineRunsTimelineProps {
  ...
  readonly rows: readonly CaravanPipelineRunStatsByDayRow[];
  /** ダークモード。省略時は false。 */
...
export function mountPipelineRunsTimeline(
  ...
  root.setAttribute('aria-label', props.t('caravan.runs.timeline'));
  ...
  emptyEl.textContent = props.t('caravan.runs.empty');

  function buildSpec(rows: readonly CaravanPipelineRunStatsByDayRow[]) {
  ...
  function renderChart(rows: readonly CaravanPipelineRunStatsByDayRow[]): void {
