/**
 * BugCausal サイドパネルの vanilla 版（`components/caravan/BugCausalPanel.tsx` の素 DOM 等価）。
 *
...
import type { CaravanBugCausalInfo } from '../../data/types';
...
export function mountBugCausalPanel(
  ...
  // --- 状態 ---
  type State = { kind: 'empty' } | { kind: 'loading' } | { kind: 'data'; info: CaravanBugCausalInfo };
  ...
  function renderContent(): void {
    ...
    if (state.kind === 'loading') {
      ...
      msg.style.cssText =
        'height:100%;display:flex;align-items:center;justify-content:center;font-size:0.75rem;color:var(--am-color-text-secondary);';
      msg.textContent = props.t('caravan.loading');
      root.appendChild(msg);
      return;
    }
