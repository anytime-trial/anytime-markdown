 ...
 *
 * 設計の要点:
 *   - `CaravanReader` は props.serverUrl から作り、`update()` で serverUrl が変わったら作り直す
...
import type { CaravanDriftEventRow } from '../../data/types';
...
export function mountDriftSection(
  ...
  let reader = new CaravanReader(props.serverUrl);
  let rows: readonly CaravanDriftEventRow[] = [];
  ...
  return {
    update(next) {
      ...
      if (serverUrlChanged) {
        ...
        reader = new CaravanReader(props.serverUrl);
        rows = [];
        historyPoints = [];
        load();
