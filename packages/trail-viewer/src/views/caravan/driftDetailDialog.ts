/**
 * DriftDetailDialog の vanilla 版（`components/caravan/DriftDetailDialog.tsx` の素 DOM 等価）。
 *
...
import type { CaravanDriftEventDetail } from '../../data/types';
...
export function mountDriftDetailDialog(
  ...
  let props = initial;
  let detail: CaravanDriftEventDetail | null = null;
  let loading = true;
  let note = '';
  let resolving = false;
  ...
  void props.onLoadDetail(props.eventId).then((d) => {
    if (cancelled) return;
    detail = d as CaravanDriftEventDetail | null;
    loading = false;
    render();
  });
