...
describe('trail viewer tab definitions', () => {
  it('does not include the legacy Releases / Prompts tabs in top-level tabs', () => {
    const tabs = getTrailViewerTabDefs({ hasC4: true, hasTrace: true });

    expect(tabs.map((tab) => tab.value)).toEqual([0, 1, 4, 5]);
  ...
  it('normalizes legacy initialTab values (3=releases / 2=prompts) to Activity', () => {
