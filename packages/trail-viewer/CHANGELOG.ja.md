...
- memory-core の知識グラフを共起ネットワークで表示する「知識グラフ」タブを追加した。
- コードグラフタブに Time Scrubber と Auto Playback を追加し、コミット粒度で State Replay を再生できるようにしました。目盛りはコミット単位で取得し、スクラバはズームできます。
- コードグラフタブに前版との差分表示を追加しました。
- コードグラフを最終編集者・編集頻度で着色できるようにしました。

### 修正

- 再生の停止を、トグルではなく停止専用の経路へ分けました。
- 差分 prop が変化したときに sigma グラフを再構築するようにしました。
- ベースラインの取り違えと、コミット一覧の縮退表示を修正しました。
- セッション配色をパレット数で打ち切るようにしました。

### 追加

- 分析タブのトークンチャートに、トークン / コストに加えて LOC（変更行数）モードを追加しました。日次の追加 LOC・削除 LOC を、トークン消費と同じ時間軸の積み上げ棒として描画します。

### 変更

- 分析グラフの集計単位トグルの隣にワークスペース切替（All + 正規化名一覧）を追加しました。切替時はそのワークスペースへ絞った combined データを再取得し、全体集計由来の品質 / コスト最適化オーバーレイは誤読防止のため非表示にし、日付ドリルダウンも該当ワークスペースのセッションのみに絞ります（worktree は親ワークスペースへ合算）。
- リポジトリ別チャート（`repos` トグル・`ReposCombinedChart`・`repoStats`）を廃止し、ワークスペース切替に置き換えました。

### 修正

- エージェント系列のブランド色が、集計側で畳んだ表示ラベル（'Claude Code' / 'Codex'）を `sessions.source` のキーとして参照していたため常に汎用パレットへフォールバックしていた問題を修正しました。
- 凡例の欠損率表示が、ターン単位の統計を持たないデータソース（Supabase 経路は `tokenTotalTurns` を 0 固定で返す）でも「欠損 0%」と誤って断言していた問題を修正し、実際に統計があるときのみ表示するようにしました。

### 追加

- 分析パネルの期間を日数入力へ変更し、集計単位の 1day / 1week トグルを追加しました。
- データ 0 件でも円グラフを一定サイズで描画するようにしました。

### 変更

- Bus Factor の C4 集約をサーバー側へ移し、ビューアと同じモデル解決経路を使うようにしました。

### 修正

- 使用量カードの高さを他のカードと揃えました。
- 期間入力が再描画で編集状態を失う問題と、フェッチ競合を解消しました。
- 属人度の C4 モデル解決可否を対象切替時にリセットするようにしました。

### 追加

- C4: `classifyLayer` による層注釈・可視化（code graph 配色＋C4 architecture-layer オーバーレイ）。
- C4 選択要素 詳細パネルにアーキテクチャ層を表示。
- Scatter / Code Graph ポップアップの vanilla 配線を完成。

### 変更

- C4 要素ツリーの種別バッジを SVG アイコン化（S / C / Co のテキスト廃止）。
- C4 左パネルのコントロール（Ghost Edges・Hotspot・オーバーレイ凡例）を高さ上限と内部スクロール付きの 1 列に集約。
- C4 右パネル（選択要素 / コミュニティ）のフォントを他パネルに揃えて拡大。
...
- C4 Matrix の C2 / C3 / C4 切替で表データを更新するように修正。
- C4 ポップアップに `pointer-events: auto` を付与し操作不能を解消。
- code graph ポップアップを閉じたら検索 / 選択状態をリセット。
- C4 ポップアップの未定義 i18n キーを補完（生キー表示を解消）。
...
- C4 オーバーレイ凡例: coverage・importance・defect-risk・dead-code・DSM-cyclic・サイズ系メトリクスを離散セグメントバーに統一
- C4 オーバーレイ凡例: coverage を 3 色連続グラデーション横バーに変更
- C4 オーバーレイ凡例: サイズ系閾値ラベルを境界位置に表示し、左端に最大値・右端に 0 を配置
- C4 オーバーレイ凡例: 全オーバーレイで左→右に値が大きくなる方向に統一
- C4 オーバーレイ凡例: 各オーバーレイタイプに `?` アイコンとヘルプ Tooltip を追加
- Hotspot: popup に churn メトリクスを追加し、defect-risk/hotspot のゲートを削除
- 要素ツリー検索ボックスにクリアアイコンを追加
- メッセージタブフィルタの幅を C4 model タブと統一し、クリアボタンを追加
- オーバーレイセレクタ: リポジトリ選択プルダウンを昇順ソートに変更
- オーバーレイセレクタ: データ非依存時のオーバーレイを disabled 制御し、DSM を既定に変更

### 修正

- Hotspot overlay の凡例が表示されない問題を修正
- MetricsReader の Materialized View クエリ URL 長超過の回避策を追加

### 変更

- MatrixPanel から DSM ビューを削除し、popup メトリクスを overlay と整合
- Hotspot から `subagent` 粒度オプションを削除
