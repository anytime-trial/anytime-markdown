...
describe('linkByC4Scope', () => {
  ...
  test('シナリオ1: pkg_ の c4Scope が 1 件解決されること', async () => {
    const { db, cleanup } = await openFreshWithTrailRows([
      { id: 'pkg_trail-caravan-book', name: 'trail-caravan-book' },
    ]);
    try {
  ...
  test('pkg_pkg/component プレフィックスの c4Scope が Concept type として解決されること', async () => {
    const { db, cleanup } = await openFreshWithTrailRows([
      { id: 'pkg_trail-caravan-book/engine', name: 'engine' },
    ]);
    try {
      ...
      const result = linkByC4Scope({
        ...
        specDocId,
        specEntityId,
        c4Scope: ['pkg_trail-caravan-book/engine'],
        recordedAt: TS,
        logger,
      });
  ...
  test('複数 c4Scope: 1 件解決 + 1 件未知 → resolved=1, skipped=1', async () => {
    ...
    try {
      ...
      const result = linkByC4Scope({
        ...
        specDocId,
        specEntityId,
        c4Scope: ['pkg_trail-caravan-book', 'com_unknown'],
        recordedAt: TS,
        logger,
      });
