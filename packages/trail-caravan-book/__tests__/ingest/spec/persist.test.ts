...
function makeClaim(overrides: Partial<Claim> = {}): Claim {
  return {
    subject: { name: 'trail-caravan-book', type: 'Package' },
    predicate: 'uses',
    ...
    try {
      const parsed = makeParsedSpec({
        frontmatter: {
          ...
          date: '2026-05-01',
          updated: undefined,
          c4Scope: ['pkg_trail-caravan-book', 'sys_anytime-markdown'],
          lang: 'ja',
        },
      });
      ...
      expect(c4Json).toEqual(['pkg_trail-caravan-book', 'sys_anytime-markdown']);
    } finally {
      db.close();
    }
  });
