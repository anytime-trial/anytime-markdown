...
describeIfNative('BetterSqlite3CaravanDb', () => {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { BetterSqlite3CaravanDb } = require('../../../src/db/connection/BetterSqlite3CaravanDb');

  test(':memory: で open でき、CREATE/INSERT/SELECT が回る', () => {
    const conn = BetterSqlite3CaravanDb.openInCaravan();
