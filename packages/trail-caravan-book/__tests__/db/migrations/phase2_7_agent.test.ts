...
describe('Phase 2.7 migration (007_phase2_7_agent)', () => {
  ...
  test('caravan_review_runs table is created', async () => {
    const { db, close } = await openFresh();
    const result = db.exec(
      "SELECT name FROM sqlite_master WHERE type='table' AND name='caravan_review_runs'",
    );
  ...
  test("caravan_reviews.source_kind accepts 'agent'", async () => {
    const { db, close } = await openFresh();
    ...
    const count = db.exec(
      "SELECT COUNT(*) FROM caravan_reviews WHERE source_kind = 'agent'",
    );
    expect(count[0]?.values?.[0]?.[0] as number).toBe(1);
    close();
  }, 30000);

  test("caravan_reviews.source_kind still accepts 'review_doc' and 'session'", async () => {
    const { db, close } = await openFresh();
    ...
    const count = db.exec(
      "SELECT COUNT(*) FROM caravan_reviews WHERE source_kind IN ('review_doc', 'session')",
    );
    expect(count[0]?.values?.[0]?.[0] as number).toBe(2);
    close();
  }, 30000);

  test('caravan_reviews.source_kind rejects invalid value after migration', async () => {
    const { db, close } = await openFresh();
  ...
  // ── caravan_review_runs CHECK constraints ────────────────────────────────────

  test('caravan_review_runs: valid insert succeeds', async () => {
    const { db, close } = await openFresh();
    expect(() => {
      db.run(
    ...
    const count = db.exec("SELECT COUNT(*) FROM caravan_review_runs WHERE id = 'run-valid-1'");
    expect(count[0]?.values?.[0]?.[0] as number).toBe(1);
    close();
  }, 30000);

  test('caravan_review_runs: CHECK trigger_kind rejects invalid value', async () => {
    const { db, close } = await openFresh();
    expect(() => {
      db.run(
        `INSERT INTO caravan_review_runs
  ...
  test('caravan_review_runs: CHECK status rejects invalid value', async () => {
    const { db, close } = await openFresh();
    expect(() => {
      db.run(
        `INSERT INTO caravan_review_runs
  ...
  test('caravan_review_runs: CHECK prompt_kind rejects invalid value', async () => {
    const { db, close } = await openFresh();
    expect(() => {
      db.run(
        `INSERT INTO caravan_review_runs
  ...
  test('caravan_review_runs: FK review_id is nullable (NULL succeeds)', async () => {
    const { db, close } = await openFresh();
    expect(() => {
      db.run(
        `INSERT INTO caravan_review_runs
  ...
  test('caravan_review_runs: FK review_id rejects non-existent review', async () => {
    const { db, close } = await openFresh();
    expect(() => {
      db.run(
        `INSERT INTO caravan_review_runs
  ...
  test('caravan_review_runs: started_at GLOB CHECK rejects malformed timestamp', async () => {
    const { db, close } = await openFresh();
    expect(() => {
      db.run(
        `INSERT INTO caravan_review_runs
