...
/**
 * trail-caravan-book DB を open し、activity.db を read-only ATTACH した {@link MemoryDbSession}
...
export async function openMemoryDbSession(
  ...
  try {
    ...
    if (created) logger.info(`trail-caravan-book backup rotated: ${memoryDbPath}.bak.1.gz`);
  } catch (err) {
