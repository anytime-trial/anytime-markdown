...
- 共起ネットワーク図種（markdown-rich / graph-core）: anytime-graph に、ある主題事象と一緒に語られる要因事象を一枚で読める共起ネットワーク図を追加しました。円の大きさ=出現頻度、線の太さ=共起強度、色=クラスタで符号化します。
...
- screenmock 編集パネル（markdown-rich）: 編集ダイアログの右側に部品・属性・構造・画面のパネルを追加しました。バリアント・色トークン・オフセット入力・ドラッグ挿入・階層ツリー・画面の追加/複製/削除・id/title 編集に加え、スニペット・画面テンプレート・プリセット・並べ替え・整列・ズームに対応します。
...
- screenmock フェンス（markdown-rich）: `screenmock` コードフェンスが、画面遷移・トランジション・部品語彙を備えたインタラクティブな画面モックを描画します。デザイン編集モードでは部品のマウスリサイズに対応し、編集したレイアウトをソースへ書き戻します。
