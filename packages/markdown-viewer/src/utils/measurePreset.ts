 ...
 *
 * 可読推奨域（和文 35〜45 全角字 / 欧文 45〜75 字）に沿って次の 3 段を用意する。
 * - focus    : 約 40em（没入して読む。最も狭い）
 * - standard : 約 46em（既定。通常の読み書き）
 * - wide     : 約 60em（表・コード主体の文書。従来幅に近い）
 */

export type MeasurePreset = "focus" | "standard" | "wide";

/** プリセット → CSS `max-width`（em 基準）。 */
...
/** 切替 UI で提示する順序（集中 → 標準 → 広い）。 */
export const MEASURE_PRESETS: readonly MeasurePreset[] = ["focus", "standard", "wide"];

/** 既定プリセット（CSS フォールバックと一致させる）。 */
export const DEFAULT_MEASURE: MeasurePreset = "standard";

/**
 * プリセット名を CSS `max-width`（em）へ変換する。
