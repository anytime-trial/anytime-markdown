import {
  DEFAULT_DARK_H1_GRADIENT, DEFAULT_DARK_H2_BORDER, DEFAULT_DARK_H2_GRADIENT, DEFAULT_DARK_H3_BORDER,
  DEFAULT_DARK_HEADING_LINK,
  DEFAULT_LIGHT_H1_GRADIENT, DEFAULT_LIGHT_H2_BORDER, DEFAULT_LIGHT_H2_GRADIENT, DEFAULT_LIGHT_H3_BORDER,
...
function blockLabel(isDark: boolean, right = "calc(100% + 8px)") {
  return {
    position: "absolute",
    right,
    top: "50%",
...
function blockLabelTop(isDark: boolean, right = "calc(100% + 30px)") {
  return {
    ...blockLabel(isDark, right),
    top: 2,
    transform: undefined,
...
/** 見出し・ブロックラベルスタイル */
export function getHeadingStyles(isDark: boolean): Record<string, unknown> {
  return {
    "& .heading-number": {
    ...
    "& > p": {
      position: "relative",
      "&::before": { content: "'P'", ...blockLabel(isDark) },
      ...hoverShow,
    },
    "& p": { mb: 1 },
    "& > blockquote > p": {
      position: "relative",
      "&::before": { content: "'Quote'", ...blockLabelTop(isDark, "calc(100% + 30px)") },
      ...hoverShow,
    },
    "& li": {
      mb: 0.25,
      position: "relative",
      "&::before": blockLabelTop(isDark, "calc(100% + 32px)"),
      ...hoverShow,
    },
