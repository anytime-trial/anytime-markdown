...
describe("getMergeTiptapStyles", () => {
  it("returns styles without showHoverLabels (default)", () => {
    const result = getMergeTiptapStyles(false, DEFAULT_SETTINGS);
    expect(result).toBeDefined();
    expect(result["& .tiptap"]).toBeDefined();
  });

  it("includes heading block labels when showHoverLabels=true (light)", () => {
    const result = getMergeTiptapStyles(false, DEFAULT_SETTINGS, { showHoverLabels: true });
  ...
  it("hides block labels when showHoverLabels=false", () => {
    const result = getMergeTiptapStyles(true, DEFAULT_SETTINGS, { showHoverLabels: false });
    const tiptap = result["& .tiptap"] as Record<string, any>;
  ...
  it("composes code styles in dark mode", () => {
    const result = getMergeTiptapStyles(true, DEFAULT_SETTINGS, { showHoverLabels: true });
    const tiptap = result["& .tiptap"] as Record<string, any>;
  ...
  it("uses settings fontSize and lineHeight", () => {
    const result = getMergeTiptapStyles(false, { ...DEFAULT_SETTINGS, fontSize: 18, lineHeight: 2.0 });
    const tiptap = result["& .tiptap"] as Record<string, any>;
