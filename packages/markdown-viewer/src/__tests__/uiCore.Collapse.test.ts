/**
 * ui-core/Collapse.ts（素 DOM ファクトリ）の jsdom ユニットテスト。
 *
 * 検証観点（contract §6）:
 ...
 * setTimeout は jest fake timers で進める（uiCore.transitionMount.test.ts と同方式）。
 *
 * jsdom は継承された CSS カスタムプロパティを getComputedStyle で解決しないため、
 * テーマ変数の検証は cssText が var(--am-...) を含むことで行う（computed の inherit 検証は禁止）。
 */

import { createCollapse } from "@anytime-markdown/ui-core/Collapse";
