/**
 * ui-core/Backdrop.ts（素 DOM ファクトリ）の jsdom ユニットテスト。
 *
 * 検証観点（contract §6）:
...
// Dialog 系（createDialog / createDialogTitle 等）は ui-core/Dialog.ts の正規モジュールと
// uiCore.Dialog.test.ts で検証する。本ファイルは createBackdrop のみを対象とする。
