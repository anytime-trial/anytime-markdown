...
describe('computeCentrality', () => {
  ...
  it('自パッケージ内のみの edge は crossPkgIn=0、externalConsumerPkgs=0', () => {
    const result = computeCentrality(
      {
        edges: [
          {
            source: 'packages/trail-activity/src/a.ts',
            target: 'packages/trail-activity/src/b.ts',
          },
        ],
      },
      {},
    ...
    const r = result[0];
    expect(r.filePath).toBe('packages/trail-activity/src/b.ts');
  ...
  it('3 つの異なる外部パッケージからの inbound で externalConsumerPkgs=3', () => {
    const target = 'packages/trail-activity/src/b.ts';
    const result = computeCentrality(
      {
        edges: [
  ...
  it('barrel ファイル (index.ts + functionCount=0) は score が同条件の非 barrel の半分', () => {
    const barrelPath = 'packages/trail-activity/src/index.ts';
    const normalPath = 'packages/trail-activity/src/util.ts';
  ...
  it('.next/types/validator のような自動生成パスは結果に含まれない', () => {
    ...
    expect(result).toHaveLength(1);
    expect(result[0].filePath).toBe('packages/trail-activity/src/b.ts');
  });
});
