...
describe('schema repoName columns', () => {
  // Phase H-4: activity_session_commits / activity_commit_files / activity_session_commit_resolutions から repo_name 列を撤去した。
  ...
  it('CREATE_SESSION_COMMIT_RESOLUTIONS DDL is exported with repo_id composite PK (no repo_name)', () => {
    expect(CREATE_SESSION_COMMIT_RESOLUTIONS).toMatch(/CREATE TABLE IF NOT EXISTS activity_session_commit_resolutions/);
  ...
  // Phase D flip: activity_session_commits / activity_commit_files の PK が repo_id を含むよう再設計された。
  ...
  it('CREATE_INDEXES contains repo_id based indexes for commit tables', () => {
    ...
    expect(joined).toMatch(/CREATE INDEX IF NOT EXISTS idx_session_commits_repo_id_committed_at ON activity_session_commits\(repo_id, committed_at\)/);
    expect(joined).toMatch(/CREATE INDEX IF NOT EXISTS idx_session_commits_repo_id_hash ON activity_session_commits\(repo_id, commit_hash\)/);
    expect(joined).toMatch(/CREATE INDEX IF NOT EXISTS idx_commit_files_repo_id_file_path ON activity_commit_files\(repo_id, file_path\)/);
    expect(joined).toMatch(/CREATE INDEX IF NOT EXISTS idx_commit_files_repo_id_hash ON activity_commit_files\(repo_id, commit_hash\)/);
  });
});
