...
describe('trailToC4', () => {
  describe('L2 Container', () => {
    ...
    it('should resolve package name from projectRoot when path has no packages/ prefix', () => {
      ...
      expect(containers[0].name).toBe('trail-activity');
    });
  ...
  describe('L3 Component', () => {
    it('should create component elements from src subdirectories', () => {
      const graph = makeTrailGraph({
        nodes: [
          { id: 'f1', label: 'ProjectAnalyzer.ts', type: 'file', filePath: 'packages/trail-activity/src/analyzer/ProjectAnalyzer.ts', line: 0 },
          { id: 'f2', label: 'toC4Helper.ts', type: 'file', filePath: 'packages/trail-activity/src/transform/toC4Helper.ts', line: 0 },
        ],
      });
      const model = trailToC4(graph);
      const components = model.elements.filter(e => e.type === 'component');
      expect(components.some(c => c.name === 'analyzer' && c.boundaryId === 'pkg_trail-activity')).toBe(true);
      expect(components.some(c => c.name === 'transform' && c.boundaryId === 'pkg_trail-activity')).toBe(true);
    });

    it('should extract components from src/ subdirectories when projectRoot is a package', () => {
      const graph = makeTrailGraph({
        metadata: { projectRoot: '/workspace/packages/trail-activity', analyzedAt: '2026-01-01', fileCount: 2 },
        nodes: [
    ...
    it('should create L3 relationships between components', () => {
      ...
      const l3Rels = model.relationships.filter(
        r => r.from === 'pkg_trail-activity/transform' && r.to === 'pkg_trail-activity/model',
      );
    ...
    it('should not create L3 relationship for same component', () => {
      ...
      const model = trailToC4(graph);
      const l3Rels = model.relationships.filter(
        r => r.from.startsWith('pkg_trail-activity/') && r.to.startsWith('pkg_trail-activity/'),
      );
  ...
  describe('L4 Code', () => {
    ...
    it('should assign code elements to component boundary', () => {
      ...
      expect(code?.boundaryId).toBe('pkg_trail-activity/analyzer');
    });
    ...
    it('should assign root files to container boundary', () => {
      ...
      expect(code?.boundaryId).toBe('pkg_trail-activity');
    });
