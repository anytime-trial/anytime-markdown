...
const defaultElements: readonly C4Element[] = [
  { id: 'pkg_trail-activity', type: 'container', name: 'trail-activity' },
  { id: 'pkg_trail-caravan-book', type: 'container', name: 'trail-caravan-book' },
];
...
describe('checkArchitecturalAlignment', () => {
  it('skips test-only changes without findings', async () => {
    const report = await checkArchitecturalAlignment(
      makeDeps({
        files: [
          changedFile('packages/trail-activity/src/domain/usecase/__tests__/Foo.test.ts', {
            addedExportLines: 1,
          }),
          changedFile('packages/trail-activity/src/domain/usecase/Bar.spec.ts', {
            linesAdded: 50,
          }),
        ],
      }),
  ...
  it("reports stale when an exported API changes and the matching spec was not updated", async () => {
    const report = await checkArchitecturalAlignment(
      makeDeps({
        files: [
          changedFile('packages/trail-activity/src/domain/usecase/Foo.ts', {
            addedExportLines: 1,
          }),
        ],
        specsByElement: {
          'pkg_trail-activity': [
            { specPath: 'spec/31.trail/02.trail-activity/trail-activity.ja.md', c4Scope: ['pkg_trail-activity'] },
          ],
        },
      }),
      sessionInput,
    ...
    expect(report.findings).toEqual([
      {
        ...
        specPath: 'spec/31.trail/02.trail-activity/trail-activity.ja.md',
        changedFiles: ['packages/trail-activity/src/domain/usecase/Foo.ts'],
  ...
  it("reports ok when an exported API changes and the matching spec was updated", async () => {
    const specPath = 'spec/31.trail/02.trail-activity/trail-activity.ja.md';
    const report = await checkArchitecturalAlignment(
      makeDeps({
        files: [
          changedFile('packages/trail-activity/src/domain/usecase/Foo.ts', {
        ...
        specsByElement: {
          'pkg_trail-activity': [{ specPath, c4Scope: ['pkg_trail-activity'] }],
        },
  ...
  it('emits one finding per spec linked to the same elementId', async () => {
    const report = await checkArchitecturalAlignment(
      makeDeps({
        ...
        specsByElement: {
          'pkg_trail-activity': [
            { specPath: 'spec/31.trail/02.trail-activity/api.ja.md', c4Scope: ['pkg_trail-activity'] },
            { specPath: 'spec/31.trail/02.trail-activity/design.ja.md', c4Scope: ['pkg_trail-activity'] },
          ],
        },
      }),
      sessionInput,
    ...
    expect(report.findings.map((finding) => finding.specPath)).toEqual([
      'spec/31.trail/02.trail-activity/api.ja.md',
      'spec/31.trail/02.trail-activity/design.ja.md',
    ]);
  });
  ...
  it('sorts findings deterministically by elementId and specPath', async () => {
    const report = await checkArchitecturalAlignment(
      makeDeps({
        files: [
          ...
          changedFile('packages/trail-caravan-book/src/ingest/spec/Foo.ts', {
            addedExportLines: 1,
          }),
        ],
        specsByElement: {
          'pkg_trail-activity': [
            { specPath: 'spec/31.trail/02.trail-activity/z.ja.md', c4Scope: ['pkg_trail-activity'] },
            { specPath: 'spec/31.trail/02.trail-activity/a.ja.md', c4Scope: ['pkg_trail-activity'] },
          ],
          'pkg_trail-caravan-book': [
            { specPath: 'spec/31.trail/04.trail-caravan-book/trail-caravan-book.ja.md', c4Scope: ['pkg_trail-caravan-book'] },
          ],
        },
      }),
      sessionInput,
    ...
    expect(
      ...
      {
        elementId: 'pkg_trail-activity',
        specPath: 'spec/31.trail/02.trail-activity/a.ja.md',
      },
      {
        elementId: 'pkg_trail-activity',
        specPath: 'spec/31.trail/02.trail-activity/z.ja.md',
      },
      {
        elementId: 'pkg_trail-caravan-book',
        specPath: 'spec/31.trail/04.trail-caravan-book/trail-caravan-book.ja.md',
      },
    ]);
  });
});
