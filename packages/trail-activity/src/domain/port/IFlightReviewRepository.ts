...
export interface IFlightReviewRepository {
  /**
   * 機械集計行の UPSERT（activity.db へ書き込む副作用を持つ）。
  ...
  /**
   * 機体の自己評価を反映（activity.db へ書き込む副作用を持つ。Phase 6 S2）。
  ...
  /** 学習候補を保存（activity.db へ書き込む副作用を持つ。Phase 6 S2）。 */
  ...
  /**
   * 手動訂正の部分更新（activity.db へ書き込む副作用を持つ。Phase 6 S3）。
  ...
  /**
   * Rationale Audit の監査ステータスを記録（activity.db へ書き込む副作用を持つ。Phase 6 S4）。
