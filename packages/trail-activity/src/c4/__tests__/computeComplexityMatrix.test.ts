...
const elements: C4Element[] = [
  ...
  { id: 'pkg_trail-activity', type: 'container', name: 'Trail Core' },
];
...
describe('computeComplexityMatrix', () => {
  ...
  it('高複雑度メッセージの最多/最高を正しく計算', () => {
    const messages: MessageInput[] = [
      { outputTokens: 5000, toolCallNames: ['Edit', 'Read', 'Grep'], editedFilePaths: ['packages/trail-activity/src/a.ts', 'packages/trail-activity/src/b.ts', 'packages/trail-activity/src/c.ts'] },
    ];
    const result = computeComplexityMatrix(messages, elements);
    const entry = result.entries.find(e => e.elementId === 'pkg_trail-activity')!;
