...
describe('getPackagePrefix', () => {
  it('extracts packages/<name>/ prefix', () => {
    expect(getPackagePrefix('packages/trail-activity/src/foo.ts')).toBe('packages/trail-activity/');
  });
...
describe('buildCallHierarchyNodeFilter', () => {
  ...
  it('excludes test files when excludeTests=true', () => {
    const f = buildCallHierarchyNodeFilter({
      scope: 'project',
      excludeTests: true,
      rootFilePath: 'packages/trail-activity/src/a.ts',
    });
    expect(f).toBeDefined();
    expect(f!(node('packages/trail-activity/src/a.ts'))).toBe(true);
    expect(f!(node('packages/trail-activity/src/a.test.ts'))).toBe(false);
    expect(f!(node('packages/trail-activity/src/__tests__/a.ts'))).toBe(false);
  });
  ...
  it('restricts to same packages/<name>/ prefix for scope=package', () => {
    const f = buildCallHierarchyNodeFilter({
      scope: 'package',
      excludeTests: false,
      rootFilePath: 'packages/trail-activity/src/foo.ts',
    });
    expect(f!(node('packages/trail-activity/src/bar.ts'))).toBe(true);
  ...
  it('combines scope and excludeTests', () => {
    ...
    expect(f!(node('packages/trail-activity/src/bar.ts'))).toBe(true);
    expect(f!(node('packages/trail-activity/src/bar.test.ts'))).toBe(false);
    expect(f!(node('packages/trail-viewer/src/x.ts'))).toBe(false);
  });
});
