// packages/trail-activity/src/analyzer/ExportExtractor.ts
...
export class ExportExtractor {
  ...
  private static visitTopLevel(
    ...
    if (ts.isVariableStatement(node)) {
      collectExportedFunctionVars(node, filePath, line, out);
      return;
    }
    if (ts.isClassDeclaration(node) && node.name) {
      out.push({ id: `${filePath}::${node.name.text}`, name: node.name.text, kind: 'class', filePath, line });
      collectPublicMethods(node as ts.ClassDeclaration & { name: ts.Identifier }, sf, filePath, out);
    }
  }
}

function collectExportedFunctionVars(
...
function collectPublicMethods(
  node: ts.ClassDeclaration & { name: ts.Identifier },
  sf: ts.SourceFile,
  filePath: string,
  out: ExportedSymbol[],
): void {
  for (const member of node.members) {
    ...
    const isPrivate = memberMods?.some(
