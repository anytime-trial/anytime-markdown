...
describe('loadAnalyzeExclude', () => {
  it('returns an Ignore that matches nothing when .anytime/trail/analyze-exclude is missing', () => {
  ...
  it('parses .gitignore-style content (directory, file glob, negation)', () => {
    const root = makeTempDir();
    try {
      fs.mkdirSync(path.join(root, '.anytime', 'trail'), { recursive: true });
      fs.writeFileSync(
        path.join(root, '.anytime', 'trail', 'analyze-exclude'),
  ...
  it('handles root-anchored patterns (/dist)', () => {
    ...
    try {
      fs.mkdirSync(path.join(root, '.anytime', 'trail'), { recursive: true });
      fs.writeFileSync(path.join(root, '.anytime', 'trail', 'analyze-exclude'), '/dist\n');
      const ig = loadAnalyzeExclude(root);
  ...
  it('returns empty Ignore when file is empty', () => {
    ...
    try {
      fs.mkdirSync(path.join(root, '.anytime', 'trail'), { recursive: true });
      fs.writeFileSync(path.join(root, '.anytime', 'trail', 'analyze-exclude'), '');
      const ig = loadAnalyzeExclude(root);
