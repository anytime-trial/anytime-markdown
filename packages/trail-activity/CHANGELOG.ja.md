...
- 本パッケージの変更なし。Anytime Trail 拡張 1.6.0 とバージョンを揃えた。

## [1.5.4] - 2026-09-23

### 修正

- 世代で単価が変わったモデルのコストを正しい単価で計算するようにした。料金表が family 単位（opus / fable / sonnet）で、同じ family 内の単価改定を反映できず、2026-08-24〜09-23 の 30 日を 16,477 USD と計算していた（正しくは 15,001 USD）。Opus 5.5（$4/$20、キャッシュ読取 $0.20）、Fable 5.1（キャッシュ読取 $0.25）、Sonnet 5（$2/$10）を世代で選ぶ。集計キーは family のまま据え置き、Opus 占有率などの時系列が世代交代で別系列へ割れないようにした。
- `claude-opus-5-5-latest` や `claude-fable-5-1[1m]` のような接尾辞付きの ID が、警告も出さず family の旧単価へ落ちていたのを直した。

## [1.5.3] - 2026-09-13

### 修正

- `minIso` / `maxIso` の `reduce` に初期値を渡すようにした。空配列で例外になる経路を塞いだ。
- `debrief` フェンスの正規表現で `\s*` が後続の改行と重なって super-linear になっていたのを解消した（Sonar S8786）。

## [1.5.2] - 2026-09-01

### 修正

- 非モノレポの C4 レベル割当が 1 段ずれていた。`trailToC4` のレベル割当が `packages/` プレフィックス専用だったため、非モノレポでは system が生成されず、リポジトリ全体が 1 container、トップディレクトリが component になっていた。system（C1）を常時生成し、非モノレポではトップディレクトリを container（C2）、その配下（`src` は透過）を component（C3）へ割り当てる。`src` トップとリポジトリ直下のファイルは従来どおりリポジトリ名 container のままで、単一プロジェクトリポジトリの互換を保つ。

## [1.5.0] - 2026-08-22

### 変更

- trail リリースセットとバージョンを同期するための更新。本パッケージの機能変更はない。
...
- trail 系の同期セットに合わせてバージョンを更新した。パッケージ自体の機能変更はない。

## [1.2.0] - 2026-08-17

### 追加

- 未確定論点への回答を 1 行 1 論点で記録するテーブル `caravan_doctrine_point_resolutions` を追加した（DCT-19）。申告列は書き換えず、解消は加算のみで表現する。

## [1.1.0] - 2026-08-11

### 変更

- アクティビティ取込とコードグラフ照会の高複雑度関数を分割した（SonarCloud S3776 の 10 件）。挙動は変えていない。

### 修正

- SonarCloud high 指摘を解消した（比較関数なしの `sort`、冗長な `void`、非推奨 API 呼び出し）。
