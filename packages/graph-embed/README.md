...
```bash
npm install @anytime-markdown/graph-embed
```

```js
import '@anytime-markdown/graph-embed'; // import するだけで <anytime-graph> が登録される
```
...
```html
<script type="module"
  src="https://cdn.jsdelivr.net/npm/@anytime-markdown/graph-embed/dist/anytime-graph.js"></script>
```
...
`npm run build --workspace=@anytime-markdown/graph-embed` で生成した `dist/anytime-graph.js`（ESM）または `dist/anytime-graph.iife.js` を相手アプリにコピーして読み込む。
...
<script type="module">
  import '@anytime-markdown/graph-embed';
...
バリデーション用の JSON Schema（Draft 2020-12）をサブパス `@anytime-markdown/graph-embed/schema`（= `graph-input.schema.json`）で公開している。
CDN なら `https://cdn.jsdelivr.net/npm/@anytime-markdown/graph-embed/graph-input.schema.json` から取得できる。
...
```bash
npm run build --workspace=@anytime-markdown/graph-embed   # dist 生成（ESM + IIFE + .d.ts）
npm run typecheck --workspace=@anytime-markdown/graph-embed
npm test --workspace=@anytime-markdown/graph-embed
```
