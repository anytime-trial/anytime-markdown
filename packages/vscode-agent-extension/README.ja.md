...
[**プロジェクトサイトを見る**](https://www.anytime-trial.com)
...
Codex には agent-status のライフサイクルフックが無いため、**最終アクティビティ**と**コンテキストトークン**（⚠️ バッジ）のみ表示し、編集中ロック・コミット数は Claude 専用です。一方 **新セッションへの引き継ぎ**は Codex セッションにも対応します（§4）。右クリックメニューは **新セッションへ引き継ぎ**・**セッション ID をコピー**。**今日**の集計は Claude 専用で *Today (Claude)* と表示します。グループの表示は `anytimeAgent.showCodexSessions` で切り替えられます。
...
- **Claude セッション**: ターミナルで新しい `claude` セッションを起動し、引き継ぎ内容を自動注入（ワンクリック起動）。または引き継ぎドキュメントのパスをコピーして新セッションの冒頭に貼り付け（クリップボードフォールバック）
- **Codex セッション**: 選択したセッションの rollout だけを読み、`codex fork`/`codex resume` ではなく `codex [PROMPT]` で、元セッションの記録済み作業ディレクトリに新しい Codex セッションを起動する。引き継ぎ本文はシェルへ文字列連結せず argv として渡すため、rollout 由来の文字列がシェルに再解釈されることはない。rollout 不在・作業ディレクトリ不在・Codex CLI 起動不能はエラー表示のみで新規ターミナルは作らない（クリップボードフォールバックは Claude と共通）
