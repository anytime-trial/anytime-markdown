...
Q5 の回答により Phase 4.5（Apply Design Tokens）の挙動が分岐する（第 8 章参照）。
...
`--design-url` または `--design-file` 指定時は Q5 をスキップし、Phase 4.5 で参照源として使用する。
...
flowchart TD
    ...
    P1["Phase 1: Interview<br/>(skill 本体)<br/>5 問 → 回答収集<br/>→ requirements.md 生成"] --> P2
    P2["Phase 2: Plan<br/>superpowers:writing-plans 呼び出し<br/>(requirements.md + stacks/t3-default.md)"] --> P3
    P3{"Phase 3: User Approval<br/>プラン承認?"}
    P3 -- No --> P2
    P3 -- Yes --> P4
    P4["Phase 4: Scaffold<br/>(skill 本体)<br/>git clone anytime-lab → リネーム<br/>→ T3 重ね合わせ + Prisma schema 反映"] --> P45
    P45["Phase 4.5: Apply Design Tokens<br/>(skill 本体 + design-md スキル)<br/>URL/DESIGN.md → tailwind.config.ts + 基底 CSS"] --> P5
...
| Phase | 実装場所 | 主要処理 |
| --- | --- | --- |
| 1. Interview | skill 本体 | `questions.md` を読み込み 5 問実施、回答を `requirements.md` に整形 |
| 2. Plan | `superpowers:writing-plans` 呼び出し | 要件 md とスタック定義を渡して実装プラン生成 |
| 3. User Approval | skill 本体 | プランをユーザに提示し承認待ち |
| 4. Scaffold | skill 本体 | `anytime-lab` クローン + リネーム + T3 重ね合わせ + Prisma schema 反映 |
| 4.5. Apply Design Tokens | skill 本体 + `design-md` | URL / DESIGN.md からデザイントークン抽出、`tailwind.config.ts` と `globals.css` に反映 |
...
| 項目 | 要件 |
| --- | --- |
| 実行時間 | 5 問インタビュー含めて 1 セッション 30 分以内（インタビュー 5 分 + プラン承認 5 分 + 実装 20 分） |
...
- 既存 superpowers との連携点（Phase 2 と Phase 5）と `design-md` 連携点（Phase 4.5）が明確に分離されている
