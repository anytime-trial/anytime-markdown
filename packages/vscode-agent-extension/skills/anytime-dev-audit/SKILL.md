...
| auto memory（MEMORY.md） | 先頭 200 行または 25KB | 超過分は起動時ロード対象外。整理手順は §5.1 |
...
発火条件は SessionStart フック `~/.claude/scripts/verify-settings-wiring.sh` の警告。索引 `MEMORY.md` が 20,480B（上限 25,600B の 80%）を超えたら整理候補、25,600B を超えたら**末尾のメモリが無言で切り捨てられている**ため即対応する。行数では検知できない（1 行 250B 前後と幅がある）ためバイト数で測る。

整理するのは**索引だけ**で、メモリ本体（`memory/*.md`）は原則残す。手段は非破壊から順に選ぶ。

| 手段 | 削減の目安 | 破壊性 | 使う場面 |
| --- | --- | --- | --- |
| 1. 要約行の短縮 | 1 行 50〜80B | 非破壊 | まず全行を見直す。ただしフックを削りすぎると「どのメモリを開くべきか」が判断できず索引の用をなさない |
| 2. アーカイブ退避 | 1 行 約 250B | 非破壊（実体は残す） | 「もう踏まない」もの。完了済み機能・解決済みの一過性障害・特定バージョン限定の罠 |
| 3. 同種メモリの統合 | 1 件 約 250B ＋本文の重複解消 | 本文を編集 | 同じ性質の罠が複数行に分かれているとき。実測値と日付は落とさない |
| 4. 陳腐化メモリの削除 | 1 行 約 250B ＋実体 | **破壊的・要承認** | 事実として誤りだと確認できたものだけ |

- 退避先は `archived-entries-index.md`。**索引行だけを移し、リンク先ファイルは削除しない**（前回 2026-07-16 の退避と同じ方式。273 件がこの形で現存する）。
- 目標はロード上限ではなく**警告閾値の下**（20,000B 以下）。上限ぎりぎりに戻すと数件の追記で再発する。
- 手段 4 は「対象提示 → 承認 → 実行」の順。メモリが名指しするファイル・関数・フラグの実在確認を経ずに消さない（規約が求める実在検証は削除判断にも要る）。
- 完了後に `wc -c` で再測定し、`MEMORY.md` とアーカイブ索引の**どちらからも辿れないファイルが 0 件**であることを確認する（孤立すると実体はあるのに永久に参照されない）。
