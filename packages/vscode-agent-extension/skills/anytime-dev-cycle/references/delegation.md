...
```markdown
- 委譲結果: 雛形v2 [codex] 採用 — <1 行所感（任意）>
- 委譲結果: 雛形v2 [qwen3:8b] 差し戻し — <乖離内容を 1 行>
- 委譲結果: 雛形v2 [codex] abstain — <abstainReason の要約>
```
...
- `[model]` は委譲を実行したモデル／実行系（例: `[codex]`・`[qwen3:8b]`）を角括弧で記す。**任意**で、`anytime-dev-retro` が版数別に加えモデル別（`delegation.byModel`）でも集計し、委譲先の役割分担（§1）の見直し材料にする。省略した旧書式も後方互換で有効（モデル未指定は `(unspecified)` に集計）。
