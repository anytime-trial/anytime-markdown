...
| 長い段階タスクをサブエージェントで回す | `references/agent-rotation.md` | 文脈圧縮・回転の運用機構 |
...
この playbook は Codex・ollama・サブエージェント（`references/agent-rotation.md` の返却契約が持つ `taskStatus: "abstained"` ＋ `abstainReason`）で共通の資産である。実行系ごとに書き換えない。
