...
> **委譲は既定であって任意ではない。** メイン（Claude）が実装を持つのは dev-cycle `SKILL.md` §3 の除外条件 E1〜E5 に該当する場合だけである。該当の有無を主観で決めず `delegation-triage.cjs` の判定に従い、見送ったときは §2.2 の見送り行を残す。
...
**再委任時のモデル昇格**: 能力不足起因の差し戻し・abstain を再委任するときは、前提修正に加えて実行系を 1 段昇格する（haiku → sonnet →（opus / Codex / Claude 自作）。dev-cycle `SKILL.md` §3.1）。同一モデル・同条件の再委任はしない。昇格後も失敗したらユーザーへエスカレーションする。昇格判断の材料は §2.2 の結果行 `[model]` 記録。
...
- **見積り行（委譲前・プラン経由の委譲では必須）**: 想定モデル・出力トークン概算（k 単位）・所要時間概算（分）・タスクカテゴリ（§2.3 の語彙・空白なし 1 トークン）。初期値は §2.3 の較正表から引く。末尾の `— 根拠: ...` は必須とし、根拠には「較正表中央値の何倍か」と、その倍率の根拠（触るファイル数・新規行数など観測可能な量）を書く。
- **結果行（委譲元検証後・必須）**: 従来どおり。
- **実測行（委譲元検証後・必須）**: 統合時に観測できた実測値（サブエージェント / Codex の出力トークン概算・wall-clock）。同一プランファイル内で**直前の未ペア見積り行（同一モデル）**と機械的に対にされるため、委譲タスクの完了条件には実測 1 行の追記を含める。
- **見送り行（委譲しなかったタスク・必須）**: `SKILL.md` §3 の除外条件で Codex 委譲を見送ったタスクに 1 行残す。除外 ID と、その ID を選んだ観測事実（ファイル数・行数・重大度・検証手段の不在など）を書く。**委譲した分だけを記録すると分母が消え、委譲率を後から測れない**（2026-08-18 実測: dev-cycle 147 セッション中 codex 起動 22 件＝15%、8 月は 3.2%。この数字は Trail DB の Bash コマンド走査でしか出せなかった）。
...
見積り行の初期値。較正済みセルは実測中央値に置き、未較正セルは仮置きとして扱う。`anytime-dev-retro` が `delegation.estimates.referenceClass`（n≥5 のカテゴリ × モデル）の実測中央値と本表の乖離を検知したら改訂提案を出す（管理方針は `proposal/20260716-model-evaluation-method.ja.md`「元データの管理場所」）。カテゴリ語彙は下表の 6 つ（空白なし 1 トークン。増やすときは本表へ追記してから使う）。

| カテゴリ | haiku | sonnet | codex |
| --- | --- | --- | --- |
| 検索調査 | out≈2k / wall≈5m | out≈4k / wall≈8m | — |
| 定型実装 | — | out≈6k / wall≈10m | out≈50k / wall≈8m |
| アルゴリズム実装 | — | — | 未較正（n=1・実測 out≈145k / wall≈25m） |
| バグ調査 | — | out≈8k / wall≈20m | out≈10k / wall≈25m |
| ドキュメント生成 | — | out≈6k / wall≈12m | — |
| レビュー | — | out≈8k / wall≈15m | out≈10k / wall≈20m |

「—」はそのカテゴリで通常選ばない組（§1 の委譲先選択・dev-cycle §3.1 に従う）。標本 5 件未満のセルは判定に使わない。`定型実装 × codex` は 2026-08-01 のレトロ実測（n=5・誤差比 out 6.50 / wall 0.53）で較正し、2026-09-26 に再較正した（n=20・実測中央値 out 50.5k / wall 7.5m・誤差比 out 0.95 / wall 0.45。前回値 wall 13m は 08-19 の n=13 実測。abstain 4 件は実測行を持たず中央値に混入していない。T-33）。他のセルは依然として未較正の仮置きである。

出力トークンと所要時間は独立に評価する。出力トークンはタスク規模が、所要時間は環境要因（起動・ビルド・テスト実行）が支配するため、片方の誤差でもう片方を補正しない。

ollama 系は実証済みタスクが限られるため本表に置かず、委譲時に都度見積る。
...
Codex が指示ファイルとして読むのはリポジトリの `AGENTS.md`（と存在すれば `~/.codex/AGENTS.md`）だけである。`~/.codex/rules/` は実行許可の prefix ルール（`default.rules`）で指示ファイルではなく、CLAUDE.md や `~/.claude/rules/` は読まれない（2026-09-13 実測）。AGENTS.md には工程・ゲートと Codex 単独実行時の読み替えを置いてあるが、現在の Claude セッションで暗黙的に共有されている文脈（直前に直したバグ・進行中のリファクタ等）は継承しない。委任プロンプトに明示的に書く。
...
| `delegation-triage.cjs` | 段4 の実行手段判定（客観入力 → `codex` / `main[EN]`）。`SKILL.md` §3 の除外条件を機械化したもの |
