...
| 定型実装 | — | out≈6k / wall≈10m | out≈65k / wall≈13m |
...
「—」はそのカテゴリで通常選ばない組（§1 の委譲先選択・dev-cycle §3.1 に従う）。標本 5 件未満のセルは判定に使わない。`定型実装 × codex` は 2026-08-01 のレトロ実測（n=5・誤差比 out 6.50 / wall 0.53）で較正済み。他のセルは依然として未較正の仮置きである。
