...
### 2.2. 見積り・結果・実測の記録（委任成績とコスト予実の測定）

プランファイルの該当タスク直下に、委譲の前後で次の行を記録する（プランなしの直接委任は結果行のみ当日の作業記録・レポートに同書式で残す。見積り・実測はプラン経由の委譲が対象）。`anytime-dev-retro` の grounding が `plan/*.md` からこれらの行を集計し、雛形版数別・モデル別の差し戻し率と、カテゴリ × モデル別の見積り誤差（`delegation.estimates.referenceClass`）を測る（`proposal/20260716-prompt-feedback-loops.ja.md` / `proposal/20260716-model-evaluation-method.ja.md`）。

- **見積り行（委譲前・プラン経由の委譲では必須）**: 想定モデル・出力トークン概算（k 単位）・所要時間概算（分）・タスクカテゴリ（§2.3 の語彙・空白なし 1 トークン）。初期値は §2.3 の較正表から引く。
- **結果行（委譲元検証後・必須）**: 従来どおり。
- **実測行（委譲元検証後・推奨）**: 統合時に観測できた実測値（サブエージェント / Codex の出力トークン概算・wall-clock）。同一プランファイル内で**直前の未ペア見積り行（同一モデル）**と機械的に対にされるため、見積りを書いたタスクには実測も書く。

書式（行頭固定。grounding の正規表現が機械読取する契約 — 変更時は `grounding.cjs` も追随）:

```markdown
- 委譲見積: [codex] out≈8k / wall≈15m / カテゴリ=定型実装 — 根拠: 類似3件の中央値
- 委譲結果: 雛形v3 [codex] 採用 — <1 行所感（任意）>
- 委譲実測: [codex] out≈12k / wall≈22m
- 委譲結果: 雛形v3 [qwen3:8b] 差し戻し — <乖離内容を 1 行>
- 委譲結果: 雛形v3 [codex] abstain — <abstainReason の要約>
```
...
見積り行の初期値。**全値が未較正の仮置き**であり、`anytime-dev-retro` が `delegation.estimates.referenceClass`（n≥5 のカテゴリ × モデル）の実測中央値と本表の乖離を検知したら改訂提案を出す（管理方針は `proposal/20260716-model-evaluation-method.ja.md`「元データの管理場所」）。カテゴリ語彙は下表の 5 つ（空白なし 1 トークン。増やすときは本表へ追記してから使う）。

| カテゴリ | haiku | sonnet | codex |
| --- | --- | --- | --- |
| 検索調査 | out≈2k / wall≈5m | out≈4k / wall≈8m | — |
| 定型実装 | — | out≈6k / wall≈10m | out≈8k / wall≈15m |
| バグ調査 | — | out≈8k / wall≈20m | out≈10k / wall≈25m |
| ドキュメント生成 | — | out≈6k / wall≈12m | — |
| レビュー | — | out≈8k / wall≈15m | out≈10k / wall≈20m |

「—」はそのカテゴリで通常選ばない組（§1 の委譲先選択・dev-cycle §3.1 に従う）。ollama 系は実証済みタスクが限られるため本表に置かず、委譲時に都度見積る。
