...
description: "分析と分析結果の出力。対象を調査・分析し、提案書（RFC / ADR / 軽量提案）または共起グラフ（.cooc.json）を出力する。「分析して」「提案書を作成」「proposal」「改善提案」「技術選定の提案」「RFC を書いて」「ADR を書いて」「設計判断を記録」「再発防止策をまとめて」で使用。「なぜなぜ分析」「原因分析」「根本原因」「ロジックツリー」「特性要因図」は因果分解の規律（§2.3）、「共起グラフを作って」「共起ネットワーク」「cooc にまとめて」は共起グラフ出力モード（§9）。How でなく Why/What を扱う。`--deep` / 「反証付きで」で反証モード、`--help` でヘルプ。"
---
