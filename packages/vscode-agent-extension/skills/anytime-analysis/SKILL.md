...
4. **根拠の解決検査**（自己申告で終わらせず、機械的に確かめる。§7.1）
5. `bash ~/.claude/scripts/validate-markdown.sh <出力ファイル>` を実行し、NG があれば修正して再実行する（`anytime-markdown-check` スキル）
6. 設計判断を含む場合は `clarity` 評価（1〜100）と理由をチャットで通知し、frontmatter にも記載する
