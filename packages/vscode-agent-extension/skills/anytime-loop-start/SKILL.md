...
> **本節の実行体はサブエージェント（`model: sonnet` 明示）**であり、メインセッションではない。tick は固定手順のみで高い推論力を要さないため、メインセッションのモデルが高コスト（Opus 等）でも本節がそのモデルで走らないようにする。メインセッションが自ら行うのは §0（前提解決・ループ自己確保）とサブエージェント起動、返却された報告の中継のみ。起動パラメータと委任プロンプトは §5.1 を参照。
...
- **メインセッションの残コストは消えない**: cron は現在のセッション上で発火するため、tick ごとにメインセッションのモデルで「起動 → 報告中継」の 1 往復は必ず発生する。これを含めて安くするにはユーザーが `/model sonnet` へ切り替えるほかない。メインセッションが高コストモデル（Opus 等）だと自覚した場合は、tick 報告に「モデル切替推奨（/model sonnet）」を 1 行含める。
