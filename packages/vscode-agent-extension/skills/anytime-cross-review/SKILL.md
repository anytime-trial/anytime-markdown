...
- **Codex 指摘** → `/Shared/anytime-markdown-docs/review/<YYYYMMDD>-<branch-slug>-codex.ja.md` を作成:
...
- `/Shared/anytime-markdown-docs/review/<YYYYMMDD>-<branch-slug>-cross-review.ja.md`（`type: report`・`<branch-slug>` は上記正規化）に出力: サマリ / 合意指摘（採用） / 係争（人手判断） / 採用アクション。**finding は再 ingest しない**（重複防止のため review でなく report）。
