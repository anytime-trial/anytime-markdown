...
test('buildReviewPrompt は anytime-trail-review とセンチネルと read-only 制約と diff 指示を含む', () => {
...
test('runReview は runCodex が throw したら ok=false・error を返す(catch パス)', async () => {
  const r = await cr.runReview({
    base: 'develop',
    runCodex: async () => { throw new Error('ENOENT'); },
    gitStatus: () => '',
    logger: { info() {}, error() {} },
  });
  assert.strictEqual(r.ok, false);
  assert.match(r.error, /ENOENT|spawn failed/);
});
