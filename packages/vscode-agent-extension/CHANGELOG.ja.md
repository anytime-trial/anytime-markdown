...
- Codex セッションにも **新セッションへ引き継ぎ** を追加した。選択した Codex セッションの rollout（`~/.codex/sessions`）だけを決定論的に読み、目的・直近状態・変更ファイル・実行コマンドを圧縮したうえで、`codex fork`/`codex resume` ではなく `codex [PROMPT]` で新しい session ID の Codex セッションを、元セッションの記録済み作業ディレクトリで起動する。引き継ぎ本文はシェルへ文字列連結せず、VS Code の `shellPath`/`shellArgs` で codex プロセスを直接起動して渡す（rollout 由来の本文にシェル制御文字が含まれても再解釈されない）。rollout 不在・読み取り不能・作業ディレクトリ不在・Codex CLI 起動不能は通知し、新規ターミナルを作らない。
...
- 同梱スキル `anytime-dev-audit`: `MEMORY.md` 索引の整理手順を追加し、上限をバイト数ではなく行数（200 行）へ訂正した。

## [1.17.0] - 2026-08-19

### 追加

- Bash 経由のファイル書き込みも airspace の claim へ記録するようにした。Edit / Write を通さないコマンド実行でも、同じファイルを触る他セッションを検知できる。
- 永続ブランチ（`master` / `main` / `develop`）への直接コミットを機械的に拒否するゲートを追加した。`develop` ブランチを持つリポジトリだけを対象とし、例外はコマンド行の脱出口で明示する。

### 変更

- 同梱スキル `anytime-dev-cycle`: 段 4（実装）の実行手段を Codex 委譲既定へ改めた。継続によるモデル復帰の実測値と、その再現期間の但し書きを回転リファレンスへ追記した。

### 修正

- Bash コマンドから書き込み対象を推定する処理を修正した（6 件）。リダイレクトと `cd` のずれ、パイプ越しの `cd`、`;` と改行が続く区切りの判定、ディレクトリ宛ての `cp` / `mv` と BSD `sed -i ""`、`cp` / `mv` のオプション値と実在判定、`-T` / `--no-target-directory` 指定時の宛先展開。
- `SendMessage` による継続でサブエージェントの model 指定が失われる問題を、回転判定で塞いだ。

## [1.16.0] - 2026-08-17

### 変更

- 同梱スキル: `anytime-dev-audit` にスキル競合スキャン観点を追加した。`anytime-analysis` に因果分解の規律（定量固定・分解軸の宣言・打ち手からの停止判定・3 軸採点）を追加し、`anytime-build-webapp` の PLAN を更新した。
...
- 緊急スプールの説明と同梱スキルの記述を、Trail のデータベース改名（`activity.db`）へ追従させた。

## [1.14.0] - 2026-08-08

### 追加

- SessionStart フックで、そのセッションがどの指示に属するかの宣言を毎回依頼するようにしました。指示とセッションの対応はこの宣言だけが作るため（先頭プロンプトの文面からは推測しません）、宣言を忘れると Flight Record 上で同じ作業が別々の行に分かれます。依頼は他セッションとの衝突有無に関わらず出します。衝突時だけ助言する airspace に相乗りさせると、単独作業のセッション（大多数）で一度も出ないためです。

### 変更

- 同梱スキル `anytime-debrief` を `anytime-session-exit` へ改名し v2 へ更新しました（出力形式ではなく発火タイミングを名前に出しました）。旧名で配置済みのコピーは初回 activate で移行します。

## [1.13.0] - 2026-08-04

### 変更

- 同梱スキル `anytime-analysis` を v10 へ更新しました（`anytime-stock-cooccurrence` への委譲記述を削除し、共起グラフ出力モードを一般形へ戻しました）。

### 修正

- airspace バンドルの導入と worktree 所有者の判定で、git を PATH 探索でなく絶対パスで起動するようにしました（SonarCloud S4036）。
...
- ソースにアクセスできない稼働中の Web アプリから、デザインシステム・UI/UX 設計思想・ユーザビリティ評価をリバースエンジニアリングする `anytime-ux-archeologist` スキルを同梱した。
- 同スキルに 48 項目のヒューリスティック評価チェックリストを追加し、宣言のみで未実装だった 4 観点を `measure-ux.js` へ実装した。

### 修正

- チェックリストの区分内訳の集計を実データへ合わせた。
- チケットの `extras` の未知キーが API / RPC 経由で消える問題を修正した。
- 同梱スキルのマージ前レビュー指摘 8 件へ対処した。

### 変更

- 拡張マニフェストに `homepage` を、README にプロジェクトサイトと他の Anytime 拡張へのリンクを追加した。
