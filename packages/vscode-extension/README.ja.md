...
- VS Code サイドバーからマークダウンドキュメントを管理
- フォルダーの追加、Git リポジトリのクローン
- 複数リポジトリ対応（各リポジトリがトップレベルノードとして表示）
- ファイル/フォルダー操作: 作成、名前変更、削除、ドラッグ&ドロップ
- リポジトリごとのブランチ切替（右クリックメニュー）
- Markdown ファイルのみ表示フィルター

### 1.2.5. 変更パネル

- Git の変更をリポジトリごとにグループ表示（ステージ済み / 未ステージ）
- ファイルごとのステージ、アンステージ、変更破棄
- コミット・プッシュ操作
- ファイル保存時の自動リフレッシュ（ファイルシステム監視）
- 変更ファイル数のバッジ表示

### 1.2.6. Git グラフ

- ブランチグラフ付きのコミット履歴表示
- カスタムアイコン: ローカルコミット（青）、リモートコミット（赤）
- ブランチ HEAD の色分けアイコン表示
- コミットメッセージ横にブランチ参照を表示
...
- 選択中ファイルの Git コミット履歴を表示
- コミットを選択して過去のバージョンを比較パネルに読み込み
