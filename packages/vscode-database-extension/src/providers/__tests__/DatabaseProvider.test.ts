...
describe('BackupTreeItem', () => {
  ...
  it('shows the backup mtime in the local time zone, not UTC', () => {
    const item = new BackupTreeItem('/db/activity.db', 1, new Date('2026-05-16T10:23:45.000Z'), 12_897_280);
  ...
  it('follows the switched time zone (no module-load-time freeze)', () => {
    ...
    const item = new BackupTreeItem('/db/activity.db', 2, new Date('2026-05-16T10:23:45.000Z'), 1024);

    expect(String(item.description)).toContain('06:23:45');
  });

  it('marks the newest generation with a distinct contextValue', () => {
    const latest = new BackupTreeItem('/db/activity.db', 1, new Date('2026-05-16T10:23:45.000Z'), 1024);
    const older = new BackupTreeItem('/db/activity.db', 2, new Date('2026-05-15T10:23:45.000Z'), 1024);
