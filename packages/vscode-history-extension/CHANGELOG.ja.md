...
- 挙動不変の複雑度リファクタが入った `trail-activity` を取り込んで再ビルドした。拡張自体の変更はない。

## [0.3.2] - 2026-08-09

### 変更

- 設計書の git 操作と git 実行ヘルパを `trail-core` → `trail-activity` のパッケージ改名へ追随させた。バンドル内容は変わるが、動作の差はない。

## [0.3.1] - 2026-08-04

### 修正

- 設計書の git 操作で、git を PATH 探索でなく絶対パスで起動するようにしました（SonarCloud S4036）。

## [0.3.0] - 2026-08-03

### 追加

- ソースジャンプ（TRC-5）: トレースの記録から対応するソース位置へ移動できるようにした。
- 既存トレースファイル向けのスキーマ v2 マイグレータ（TRC-6）を追加した。

### 修正

- `trace-agent-node` が `--require` で起動しない問題を修正した。TypeScript ソースを配布する型専用パッケージ `trace-core` を値 import していたことが原因で、`tsc`・jest・webpack はいずれも通過し `--require` 経路だけが落ちていた。
...
- タイムラインビューのコミット日時をローカルタイムゾーンで表示するようにしました。WSL の Extension Host は `TZ=UTC` で動くため、`Date` のローカル getter が UTC 値を返していました。
