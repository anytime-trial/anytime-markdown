...
function gatherConversationSource(
  ...
  try {
    ...
    if (items.length > 0 || convValue) {
      return { source: 'conversation', items: convValue ? [{ value: convValue }, ...items] : items };
    }
  } catch (err) {
...
function gatherSpecSource(
  ...
  try {
    ...
    if (items.length > 0 || specValue) {
      return { source: 'spec', items: specValue && items.length === 0 ? [{ value: specValue }] : items };
    }
  } catch (err) {
...
function gatherCodeSource(
  ...
  try {
    ...
    if (items.length > 0 || codeValue) {
      return { source: 'code', items: codeValue && items.length === 0 ? [{ value: codeValue }] : items };
    }
  } catch (err) {
...
function gatherBugHistorySource(
  ...
  try {
    ...
    if (items.length > 0) return { source: 'bug_history', items };
  } catch (err) {
...
function gatherReviewSource(
  ...
  try {
    ...
    if (items.length > 0) return { source: 'review', items };
  } catch (err) {
