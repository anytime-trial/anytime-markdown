...
- スタイル注入を `@anytime-markdown/ui-core` の `ensureStyle` 経由へ移した。
...
- database 系統のバージョン同期のための bump です。機能変更はありません。
...
- 残存していた React consumer 配線を vanilla mount へ再配線、移行経緯の @mui コメントを除去。
