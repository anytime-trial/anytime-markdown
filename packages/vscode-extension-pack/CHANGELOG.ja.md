# 変更履歴

"Anytime Extension Pack" の主な変更をこのファイルに記録します。

形式は [Keep a Changelog](https://keepachangelog.com/) に基づいています。

## [Unreleased]

### 変更

- 同梱拡張を Anytime Markdown / Anytime Graph / Anytime Trail / Anytime Agent の 4 つへ絞った。Anytime Sheet・Anytime Database・Anytime History は同梱対象外となり、個別にインストールする。
- マニフェストの `description` と README の一覧を同梱拡張と一致させた。

## [0.5.2] - 2026-08-03

### 変更

- マニフェストに `homepage` を、README にプロジェクトサイトと同梱各拡張へのリンクを追加した。

## [0.5.1] - 2026-05-17

### 変更

- 同梱拡張機能のバージョン追従: Anytime Markdown 0.15.2 / Anytime Graph 0.3.2 / Anytime History 0.2.2 / Anytime Trail 0.21.0 / Anytime Sheet 0.4.4 / Anytime Database 0.2.1 / Anytime Agent 0.2.0

## [0.5.0] - 2026-05-16

### 追加

- `anytime-trial.anytime-agent` をパックに追加

### 変更

- バンドルされている拡張機能のバージョンを更新: anytime-markdown 0.15.1、anytime-trail 0.20.0、anytime-database 0.2.0、anytime-history 0.2.1、anytime-agent 0.1.0

## [0.4.0] - 2026-05-08

### 変更

- `anytime-trial.anytime-database` をパックに追加
- バンドルされている拡張機能のバージョンを更新: anytime-trail 0.18.0、anytime-sheet 0.4.2

## [0.3.1] - 2026-05-04

### 変更

- 同梱拡張機能のバージョンを更新: anytime-markdown 0.14.0・anytime-graph 0.3.0・anytime-trail 0.16.0

## [0.3.0] - 2026-04-23

### 追加

- `anytime-trial.anytime-sheet` を extension pack に追加

## [0.2.2] - 2026-04-18

### 変更

- 拡張機能パックのアイコンを更新

## [0.2.1] - 2026-04-12

### 変更

- バンドル拡張機能 `anytime-trial.anytime-git` を `anytime-trial.anytime-history` に置き換え

## [0.2.0] - 2026-04-11

### 変更

- Anytime Git をバンドル拡張機能に追加
- バンドル拡張機能のバージョンを更新（markdown 0.11.0、graph 0.1.2、git 0.1.0、trail 0.5.1）

## [0.1.0] - 2026-04-04

### 変更

- 同梱拡張機能の Anytime Git を Anytime Trail に置き換え
- 同梱拡張機能を最新バージョンに更新（markdown 0.10.0, graph 0.1.0, trail 0.1.0）

## [0.0.3] - 2026-04-01

### 変更

- 同梱拡張機能を最新バージョンに更新（markdown 0.9.2, graph 0.0.3, git 0.0.3）

## [0.0.2] - 2026-03-29

### 変更
- パックアイコン画像を更新

## [0.0.1] - 2026-03-27

### 追加
- Anytime Markdown、Anytime Graph、Anytime Trail をバンドル
